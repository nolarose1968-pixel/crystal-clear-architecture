...
class PerformanceAnalyzer {
  ...
  public analyzeTrend(metricName: string, windowSize: number = 100): TrendAnalysis | null {
    ...
    const direction = Math.abs(slope) < 0.1 ? 'stable' :
                     Math.abs(correlation) < 0.5 ? 'volatile' :
    ...
    for (let i = segmentSize; i < windowSize - segmentSize; i += segmentSize) {
      ...
      if (Math.abs(afterMean - beforeMean) > values.reduce((a, b) => a + b, 0) / values.length * 0.2) {
  ...
  public identifyBottlenecks(): BottleneckAnalysis[] {
    ...
    this.metrics.forEach((data, metric) => {
      ...
      if (baseline) {
        ...
        const impact = degradation > 0.5 ? 'critical' :
                      degradation > 0.3 ? 'high' :
                      degradation > 0.1 ? 'medium' : 'low';
  ...
  public updateBudgets(): void {
    this.budgets.forEach((budget, metricName) => {
      ...
      if (data && data.length > 0) {
        ...
        const currentValue = recentData.map(d => d.value).reduce((a, b) => a + b, 0) / recentData.length;
  ...
  public detectRegression(metricName: string): RegressionDetection {
    ...
    const severity = changePercent > 0.5 ? 'severe' :
                    changePercent > 0.2 ? 'moderate' : 'minor';
  ...
  public generateReport(): void {
    ...
    this.budgets.forEach((budget, metric) => {
      const icon = budget.status === 'within_budget' ? '✅' : 
                  budget.status === 'approaching_limit' ? '⚠️' : '❌';
      const color = budget.status === 'within_budget' ? '\x1b[32m' : 
                   budget.status === 'approaching_limit' ? '\x1b[33m' : '\x1b[31m';
      
      console.log(`${icon} ${metric.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()).padEnd(25)} ` +
    ...
    this.metrics.forEach((data, metric) => {
      ...
      console.log(`  Mean: ${stats.mean.toFixed(2)}  Median: ${stats.median.toFixed(2)}  P95: ${stats.percentiles.p95.toFixed(2)}`);
      console.log(`  Min: ${stats.min.toFixed(2)}  Max: ${stats.max.toFixed(2)}  StdDev: ${stats.stdDev.toFixed(2)}`);
      console.log(`  Outliers: ${stats.outliers} (${((stats.outliers/stats.count)*100).toFixed(1)}%)`);
    });
    ...
    this.metrics.forEach((_, metric) => {
      ...
      if (trend) {
        ...
        console.log(`${trendIcon} ${metric.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()).padEnd(25)} ` +
                   `${trend.direction} (slope: ${trend.slope.toFixed(3)})`);
        console.log(`   Forecast: ${trend.forecast.nextValue.toFixed(1)} ` +
    ...
    if (bottlenecks.length === 0) {
      ...
      bottlenecks.forEach(bottleneck => {
        const impactIcon = bottleneck.impact === 'critical' ? '🔴' : 
    ...
    this.metrics.forEach((_, metric) => {
      ...
      if (regression.detected) {
        ...
        console.log(`${severityIcon} ${metric.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} regression detected`);
        console.log(`   Severity: ${regression.severity}`);
        console.log(`   Impact: ${regression.impactMagnitude > 0 ? '+' : ''}${regression.impactMagnitude}`);
        console.log(`   Period: ${new Date(regression.startTime).toLocaleString()} - ${new Date(regression.endTime).toLocaleString()}`);
