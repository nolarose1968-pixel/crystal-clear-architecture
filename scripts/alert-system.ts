...
class AlertSystem {
  ...
  private triggerAlert(rule: AlertRule, value: number, metadata: any): void {
    ...
    console.log(
      `🚨 Alert Triggered: ${alert.title} [${alert.severity.toUpperCase()}]`,
    );
  ...
  private sendNotifications(
    ...
    channels.forEach((channel) => {
      ...
      switch (channel.type) {
        ...
        case "pagerduty":
          console.log(
            `📟 PagerDuty incident created for ${channel.config.target}`,
          );
  ...
  private processEscalations(): void {
    this.alerts.forEach((alert) => {
      ...
      policy.levels.forEach((level) => {
        if (
          level.level > alert.escalationLevel &&
          ageMinutes >= level.delayMinutes
        ) {
          ...
          if (shouldEscalate) {
            console.log(
              `⬆️  Escalating alert ${alert.id} to level ${level.level}`,
            );
  ...
  private calculateCorrelationConfidence(alerts: Alert[]): number {
    ...
    const timeSpread =
      Math.max(...alerts.map((a) => a.triggeredAt)) -
  ...
  private buildCorrelationReason(
    commonTags: string[],
...
console.log(
  `Correlations: ${stats.correlations}, Active Suppressions: ${stats.suppressions}`,
);
...
activeAlerts.forEach((alert) => {
  ...
  console.log(
    `${alert.severity === "critical" ? "🔴" : "🟡"} ${alert.title} (${age}s ago)`,
  );
  console.log(
