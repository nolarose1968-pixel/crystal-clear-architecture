...
class MonitoringDashboard {
  ...
  private updateEndpointStatus() {
    this.endpoints.forEach((endpoint) => {
      ...
      endpoint.successRate =
        ((totalRequests - endpoint.errorCount) / totalRequests) * 100;
    });
  }
  ...
  private checkForAlerts() {
    ...
    if (currentMetrics.cpu > 80) {
      this.createAlert(
        "critical",
        "System",
        `High CPU usage: ${currentMetrics.cpu}%`,
      );
    }
    ...
    if (currentMetrics.memory > 85) {
      this.createAlert(
        "warning",
        "System",
        `High memory usage: ${currentMetrics.memory}%`,
      );
    }
    ...
    this.endpoints.forEach((endpoint, name) => {
      if (endpoint.status === "down") {
        this.createAlert(
          "critical",
          name,
          `Endpoint is down: ${endpoint.lastError}`,
        );
  ...
  private renderSystemMetrics() {
    ...
    const memStatus =
      current.memory > 85 ? "🔴" : current.memory > 70 ? "🟡" : "🟢";
    const diskStatus =
      current.disk > 90 ? "🔴" : current.disk > 80 ? "🟡" : "🟢";

    console.log(
      `${cpuStatus} CPU Usage:     ${this.renderBar(current.cpu, 100)} ${current.cpu}%`,
    );
    console.log(
