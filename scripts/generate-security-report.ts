...
class SecurityReportGenerator {
  ...
  private generateRecommendations(scanData: any[], avgRiskScore: number, complianceRate: number): string[] {
    ...
    if (unhealthyPackages.length > 0) {
      recommendations.push(`📦 UNHEALTHY PACKAGES: Consider replacing ${unhealthyPackages.join(", ")}`);
    }
  ...
  private displayReport(report: SecurityReport): void {
    ...
    console.log(`📆 Time Range: ${report.timeRange.start.toISOString().split('T')[0]} to ${report.timeRange.end.toISOString().split('T')[0]}`);
    ...
    if (report.trends.issuesOverTime.length > 0) {
      ...
      console.log(`   📅 Latest: ${latest.date} - Fatal: ${latest.fatal}, Warnings: ${latest.warnings}`);
    }
