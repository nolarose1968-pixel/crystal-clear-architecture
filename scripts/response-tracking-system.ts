...
${this.responses
  ...
  .map(r => `- **${r.name}** (${r.department}) - ${r.responseTime ? new Date(r.responseTime).toLocaleString() : 'N/A'}`)
  .join('\n')}

### **⏳ PENDING (${stats.pending})**
${this.responses
  .filter(r => r.responseStatus === 'PENDING')
  .map(r => `- **${r.name}** (${r.department}) - ${this.getTimeElapsed(r.notificationSent)} elapsed`)
  .join('\n')}

### **🚨 OVERDUE (${stats.overdue})**
${this.responses
  .filter(r => r.responseStatus === 'OVERDUE')
  .map(r => `- **${r.name}** (${r.department}) - ${this.getTimeElapsed(r.notificationSent)} elapsed - Escalation Level ${r.escalationLevel}`)
  .join('\n')}
...
**EXECUTIVE SPONSOR**: William Harris (CEO)`;
  ...
  private determineNextAction(response: TeamLeadResponse): string {
    switch (response.responseStatus) {
      ...
      case 'OVERDUE':
        return response.escalationLevel === 0 ? 'Send reminder email' : 
               response.escalationLevel === 1 ? 'Phone call follow-up' :
               'Manager escalation';
      case 'ESCALATED':
