...
export async function getMetrics(): Promise<DashboardMetrics> {
  ...
  const metrics: DashboardMetrics = {
    ...
    cpuUsage: Math.max(
      10,
      Math.min(80, 23 + Math.floor(Math.random() * 20 - 10)),
    ), // 10-80%
    memoryUsage: Math.max(
      50,
      Math.min(90, 67 + Math.floor(Math.random() * 10 - 5)),
    ), // 50-90%
    activeConnections: 342 + Math.floor(Math.random() * 100 - 50), // 292-392
    cacheHitRate: Math.max(
      70,
      Math.min(95, 87 + Math.floor(Math.random() * 8 - 4)),
    ), // 70-95%
...
export async function getAnalytics(
  ...
  for (let i = points - 1; i >= 0; i--) {
    ...
    revenue.push(
      Math.max(
        0,
        Math.floor(baseRevenue + trend + Math.random() * 10000 - 5000),
      ),
    );

    const baseUsers = 12000 + Math.sin(i * 0.3) * 3000;
    users.push(
      Math.max(0, Math.floor(baseUsers + i * 150 + Math.random() * 1000 - 500)),
    );

    engagement.push(
      Math.max(
        0,
        Math.min(100, 75 + Math.sin(i * 0.4) * 15 + Math.random() * 10 - 5),
      ),
    );
    ...
    roi.push(
      Math.max(
        0,
        Math.min(500, 200 + Math.sin(i * 0.6) * 100 + Math.random() * 50 - 25),
      ),
    );
  }
...
export async function getApiPerformance() {
  ...
  return {
    ...
    averageResponseTime:
      endpoints.reduce((sum, e) => sum + e.avgResponseTime, 0) /
      endpoints.length,
    totalRequestsPerMinute: endpoints.reduce(
      (sum, e) => sum + e.requestsPerMinute,
      0,
    ),
    timestamp: new Date().toISOString(),
  };
