...
export class HealthService {
  ...
  async getComprehensiveHealth(): Promise<ComprehensiveHealthReport> {
    try {
      ...
      const services = {
        ...
        database: this.extractResult(
          databaseHealth,
          "Database health check failed",
        ),
        external: this.extractResult(
          externalHealth,
          "External services check failed",
        ),
        application: this.extractResult(
          applicationHealth,
          "Application health check failed",
        ),
        performance: this.extractResult(
          performanceHealth,
          "Performance monitoring failed",
        ),
        cache: this.extractResult(cacheHealth, "Cache health check failed"),
        security: this.extractResult(
          securityHealth,
      ...
      const healthyServices = serviceArray.filter(
        (s) => s.status === "healthy",
      ).length;
      const degradedServices = serviceArray.filter(
        (s) => s.status === "degraded",
      ).length;
      const criticalServices = serviceArray.filter(
        (s) => s.status === "critical",
      ).length;
    ...
    for (const [serviceName, serviceData] of Object.entries(services)) {
      if (serviceData && serviceData.status !== "healthy") {
        ...
        alerts.push({
          ...
          message:
            serviceData.error ||
            `${serviceName} service is ${serviceData.status}`,
  ...
  private generateRecommendations(services: any): string[] {
    ...
    if (services.database?.status !== "healthy") {
      recommendations.push(
        "Database performance issues detected - check connection pool",
      );
    ...
    if (services.external?.status !== "healthy") {
      recommendations.push(
        "Some external services are unavailable - check network connectivity",
      );
    }
    ...
    if (services.security?.status !== "healthy") {
      recommendations.push(
        "Security vulnerabilities detected - update dependencies",
      );
