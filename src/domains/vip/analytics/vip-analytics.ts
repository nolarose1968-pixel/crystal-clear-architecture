...
export class VipAnalyticsService {
  ...
  async generateVipAnalyticsReport(
    ...
    console.log(`📊 Generating VIP analytics report for ${startDate.toISOString()} to ${endDate.toISOString()}`);
   ...
   */
  private async calculateTierAnalytics(startDate: Date, endDate: Date): Promise<VipAnalyticsReport['tierAnalytics']> {
    ...
    for (const tier of tiers) {
      ...
      const tierCustomers = customers.filter(c =>
        c.getCreatedAt() >= startDate && c.getCreatedAt() <= endDate
      );
   ...
   */
  private async calculateBenefitsAnalytics(startDate: Date, endDate: Date): Promise<VipAnalyticsReport['benefitsAnalytics']> {
    ...
    for (const customer of customers) {
      ...
      benefitsByCategory.gaming += (benefits.freeSpinsUsed || 0) + (benefits.bonusWagerMultiplierUsed || 0);
   ...
   */
  private async calculateRetentionAnalytics(startDate: Date, endDate: Date): Promise<VipAnalyticsReport['retentionAnalytics']> {
   ...
   */
  private async calculateRevenueAnalytics(startDate: Date, endDate: Date): Promise<VipAnalyticsReport['revenueAnalytics']> {
    const customers = await this.repository.findByQuery({
      createdAfter: startDate,
      createdBefore: endDate
    });
  ...
  private async generatePersonalizedRecommendations(customer: any): Promise<string[]> {
    ...
    if (tier.getBenefits().exclusiveEventsAccess && !customer.getBenefitsTracking().exclusiveEventsAttended) {
