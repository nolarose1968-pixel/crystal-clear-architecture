...
export async function exampleMonthlyReport() {
  ...
  try {
    ...
    if (response.success) {
      ...
      console.log(
        `💰 Net Profit: $${report.summary.netProfit.toLocaleString()}`,
      );
...
export async function exampleComplianceMonitoring() {
  ...
  try {
    ...
    if (analyticsResponse.success) {
      ...
      console.log(
        `   Pending Reviews: ${analytics.summary.reportsByStatus.pending_review || 0}`,
      );
      console.log(
        `   Overdue Reports: ${analytics.summary.reportsByStatus.draft || 0}`,
      );
    ...
    if (complianceResponse.success && complianceResponse.data.isCompliant) {
      ...
      if (approvalResponse.success) {
        ...
        if (publishResponse.success) {
          ...
          console.log(
            `📅 Published: ${new Date(approvedReport.publishedAt).toLocaleString()}`,
          );
...
export async function exampleAdvancedSearch() {
  ...
  try {
    ...
    if (monthlyReports.success) {
      console.log(
        `📈 Found ${monthlyReports.data.reports.length} monthly reports`,
      );
    }
    ...
    if (dateRangeReports.success) {
      console.log(
        `📊 Found ${dateRangeReports.data.reports.length} reports in last 30 days`,
      );
    }
    ...
    if (paginatedReports.success) {
      console.log(
        `📋 Page results: ${paginatedReports.data.reports.length} reports`,
      );
    ...
    return {
      ...
      dateRangeReports: dateRangeReports.success
        ? dateRangeReports.data.reports
        : [],
      paginatedReports: paginatedReports.success
...
export async function exampleMonitoringDashboard() {
  ...
  try {
    ...
    if (analyticsResponse.success) {
      ...
      console.log(`   Compliance Rate: ${analytics.summary.complianceRate}%`);
      console.log(
        `   Average Processing: ${analytics.summary.averageProcessingTime}ms`,
      );
    ...
    return {
      ...
      analytics: analyticsResponse.success
        ...
        : null,
      attentionItems: attentionResponse.success
        ? attentionResponse.data.reports
        : [],
    };
  } catch (error) {
    console.error("❌ Error:", error.message);
