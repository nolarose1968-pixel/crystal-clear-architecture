...
describe("FinancialReportingService", () => {
  ...
  beforeEach(() => {
    ...
    const repository =
      FinancialReportingRepositoryFactory.createWithMockDatabase(globalMockDb);
  ...
  describe("generateReport", () => {
    ...
    it("should handle missing optional services gracefully", async () => {
      const serviceWithoutServices = FinancialReportingServiceFactory.create(
        FinancialReportingRepositoryFactory.createWithMockDatabase(
          new MockDatabase(),
        ),
      );
  ...
  describe("publishReport", () => {
    it("should reject publishing of draft report", async () => {
      ...
      await expect(
        service.publishReport(result.report.getId()),
  ...
  describe("checkCompliance", () => {
    ...
    it("should identify critical compliance issues", async () => {
      ...
      expect(
        complianceResult.issues.some((issue) => issue.severity === "critical"),
      ).toBe(true);
    });
  });
  ...
  describe("searchReports", () => {
    ...
    it("should search reports by date range", async () => {
      ...
      reports.forEach((report) => {
        expect(report.getPeriodStart().getTime()).toBeGreaterThanOrEqual(
          startDate.getTime(),
        );
        expect(report.getPeriodEnd().getTime()).toBeLessThanOrEqual(
          endDate.getTime(),
        );
      });
    });
  ...
  describe("Integration Tests", () => {
    it("should complete full report lifecycle", async () => {
      ...
      const report = reports.find(
        (r) => r.getId() === generateResult.report.getId(),
      );
      if (report) {
        report.markForReview();
        // Update the mock database directly
        const dbReport = globalMockDb["reports"].find(
          (r) => r.id === report.getId(),
        );
        if (dbReport) {
          dbReport.status = "pending_review";
      ...
      const publishedReport = await service.publishReport(
