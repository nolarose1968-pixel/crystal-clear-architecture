...
export class DomainOrchestrator {
  ...
  async processCustomerDeposit(params: {
    ...
    try {
      ...
      if (!collectionResult.success) {
        throw new Error(
          collectionResult.error || "Collection processing failed",
        );
      }
  ...
  async processAgentBetPlacement(params: {
    ...
    try {
      ...
      console.error(
        `❌ Agent bet placement process failed: ${processId}`,
        error,
      );
      throw result;
    }
  ...
  async processCustomerOnboarding(params: {
    ...
    try {
      ...
      console.error(
        `❌ Customer onboarding process failed: ${processId}`,
        error,
      );
      throw result;
    }
  ...
  public getStats(): {
    ...
    const totalDuration = completedProcesses.reduce(
      (sum, p) => sum + p.duration,
      0,
    );
    const averageDuration =
      completedProcesses.length > 0
        ? totalDuration / completedProcesses.length
        : 0;
