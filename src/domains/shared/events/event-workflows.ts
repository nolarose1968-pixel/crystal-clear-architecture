...
export class EventWorkflows {
      ...
      steps: [
        {
          ...
          action: async (payload, context) => {
            ...
            const balanceResponse = await this.balanceController.getBalanceStatus(payload.customerId);
        ...
        },
        {
          ...
          action: async (payload, context) => {
            ...
            let bonusAmount = 0;
            if (paymentAmount >= 100) bonusAmount = paymentAmount * 0.05; // 5% bonus
  ...
  public getStats(): {
    ...
    const completedWorkflows = workflows.filter(w =>
      w.completedSteps.length > 0 && w.failedSteps.length === 0
    );
