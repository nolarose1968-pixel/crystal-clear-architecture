...
export const COLLECTIONS_ALERT_TRIGGERED = (payload: {
  alertId: string;
  alertType: 'settlement_failure' | 'high_latency' | 'payment_declined' | 'queue_overflow';
...
export function validateCollectionsMessage(message: WorkerMessage): {
  ...
  switch (message.type) {
    ...
    case 'PAYMENT_VALIDATION':
      ...
      if (!['approved', 'rejected', 'pending'].includes(message.payload.validationResult)) {
        errors.push('Valid validation result is required for PAYMENT_VALIDATION');
      }
      break;

    case 'COLLECTIONS_HEALTH_STATUS':
      if (!['healthy', 'degraded', 'unhealthy'].includes(message.payload.status)) {
        errors.push('Valid health status is required for COLLECTIONS_HEALTH_STATUS');
      }
      break;
  }
