...
class EnhancedAPITestSuite {
      ...
      for (const [path, responses] of Object.entries(mockResponses)) {
        if (endpoint === path || endpoint.startsWith(path)) {
          ...
          if (endpoint === "/api/v1/auth/login") {
            ...
            response =
              body.username === "testuser" && body.password === "testpass"
                ? responses.success
                : responses.invalid;
  ...
  async testCaching() {
    ...
    const cachedResource = this.api.cacheManager.get(
      "resource_/api/v1/protected",
    );
  ...
  async testIntegration() {
    ...
    if (!this.securityManager) {
      console.log(
        "⚠️ Security manager not available, skipping integration test",
      );
      return;
    }
    ...
    if (apiStatus.hasToken !== securityStatus.hasToken) {
      throw new Error(
        "Token status mismatch between API service and security manager",
      );
    }
