...
class WaterSystemIntegrator {
     ...
     */
    simulateUpdates() {
        this.metrics.flowRate = Math.max(50, Math.min(120, this.metrics.flowRate + (Math.random() - 0.5) * 10));
        this.metrics.temperature = Math.max(18, Math.min(26, this.metrics.temperature + (Math.random() - 0.5) * 2));
        this.metrics.pressure = Math.max(100, Math.min(140, this.metrics.pressure + (Math.random() - 0.5) * 10));
    ...
    startMonitoring(intervalMs = 5000, callback = null) {
        ...
        console.log(`🌊 Starting water system monitoring (${intervalMs}ms intervals)`);
    ...
    generateBoxDisplay() {
        ...
        // Top line with title
        display.push('╭' + '─'.repeat(leftPad) + titleStr + '─'.repeat(rightPad) + '╮');
        
        // Content lines
        display.push(`│  🌊 Flow Rate: ${this.metrics.flowRate} L/min                          │`);
        display.push(`│  🌡️ Temperature: ${this.metrics.temperature}°C                            │`);
        display.push(`│  📊 Pressure: ${this.metrics.pressure} PSI                            │`);
     ...
     */
    getStatusSummary() {
        const flowStatus = this.metrics.flowRate > 80 ? 'HIGH' : this.metrics.flowRate < 30 ? 'LOW' : 'NORMAL';
        const tempStatus = this.metrics.temperature > 24 ? 'WARM' : this.metrics.temperature < 20 ? 'COOL' : 'NORMAL';
        const pressureStatus = this.metrics.pressure > 130 ? 'HIGH' : this.metrics.pressure < 110 ? 'LOW' : 'NORMAL';
        
        return {
            flow: { value: this.metrics.flowRate, unit: 'L/min', status: flowStatus },
            temperature: { value: this.metrics.temperature, unit: '°C', status: tempStatus },
            pressure: { value: this.metrics.pressure, unit: 'PSI', status: pressureStatus },
            overall: flowStatus === 'NORMAL' && tempStatus === 'NORMAL' && pressureStatus === 'NORMAL' ? 'HEALTHY' : 'ATTENTION'
        };
    }
