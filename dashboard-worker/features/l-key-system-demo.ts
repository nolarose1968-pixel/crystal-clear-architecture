...
import { OTCMatchingEngine, OrderType, OrderSide, TradingAsset, OrderStatus } from '../services/otc-order-matching-engine';
...
export class LKeySystemDemo {
  ...
  private async demonstrateP2PTransactionFlow(): Promise<void> {
    ...
    console.log(`   Total Fee: $${processedTx.fees.totalFee.toFixed(2)} (${(processedTx.fees.effectiveRate * 100).toFixed(3)}%)`);
  ...
  private async demonstrateOTCTradingFlow(): Promise<void> {
    ...
    console.log(`   Order: ${otcRequest.side} ${otcRequest.amount.toLocaleString()} USD of ${otcRequest.asset}`);
  ...
  private async demonstrateComplexTransaction(): Promise<void> {
    ...
    console.log(`   Referral Volume: $${affiliateRequest.metadata?.referralVolume.toLocaleString()}`);
    ...
    if (originalTx) {
      const commissionAmount = originalTx.fees.totalFee * 0.30; // 30% affiliate rate
      ...
      console.log(`   Platform Revenue: $${(originalTx.fees.totalFee - commissionAmount).toFixed(2)}`);
    }
  ...
  private async demonstrateSystemAnalytics(): Promise<void> {
    ...
    console.log(`   Average Transaction Size: $${(txReport.totalVolume / txReport.totalTransactions).toLocaleString()}`);
