...
This document describes the enhanced dependency management system leveraging Bun's latest features (v1.2.21+) including `bun pm pkg`, `bun why`, improved workspace performance, and **explicit package specification with `bunx --package`** for reliable development workflows.
...
- **`bun run pkg:manage`** - Interactive package management tool with `bunx --package` examples
...
- **`bun run bundle:analyze`** - `bunx --package webpack-bundle-analyzer webpack-bundle-analyzer dist/stats.json`

#### Dependency Analysis with Enhanced Tools
- **`bun run deps:analyze`** - Comprehensive dependency analysis using `bunx --package jq` for JSON processing
...
- **`bun run security:audit`** - `bunx --package npm-audit-resolver npm-audit-resolver check`
...
- **New Priority**: devDependencies > optionalDependencies > dependencies > peerDependencies
...
1. **Explicit Package Specification** - Run binaries from packages where the binary name differs from the package name
2. **Scoped Package Support** - Easily run binaries from scoped packages like `@angular/cli`, `@nestjs/cli`
3. **Version Pinning** - Specify exact package versions for consistent execution across environments
4. **Multiple Binary Support** - Run specific binaries from packages that provide multiple executables
5. **CI/CD Reliability** - Ensures consistent behavior across different environments
...
The project includes a comprehensive CI/CD pipeline (`.github/workflows/enhanced-ci-bunx.yml`) that demonstrates best practices for using `bunx --package` in automated environments:
...
### CI/CD Benefits
1. **Reproducible Builds** - Explicit package versions ensure consistent behavior
...
This enhanced dependency management system with `bunx --package` support provides:

1. **🎯 Explicit Control** - Clear specification of which package provides each binary
...
6. **🚀 Modern Workflows** - Leverages Bun's latest features for optimal development experience

This system provides a robust foundation for maintaining Fire22's complex 15-workspace structure while leveraging Bun's latest performance and usability improvements with explicit package management.

## 📝 Editor Integration with `Bun.openInEditor()`

### Overview
Bun provides native editor integration through `Bun.openInEditor()`, allowing seamless file opening from command-line scripts and development workflows.
...
The project includes a comprehensive editor integration script (`scripts/editor-integration.ts`) that demonstrates advanced `Bun.openInEditor()` usage:
...
The most powerful feature automatically opens files with TypeScript errors at the exact error locations:
...
3. **🔄 Workflow Integration** - Seamless integration with type checking, linting, analysis
...
This editor integration enhances the development experience by providing seamless navigation between command-line tools and your editor, making the Fire22 monorepo development workflow significantly more efficient.