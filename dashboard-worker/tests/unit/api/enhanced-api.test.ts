...
async function initializeTestServer() {
  return await runScript('test-server-init', async () => {
...
async function cleanupTestServer() {
  return await runScript('test-server-cleanup', async () => {
    if (server) {
      server.stop();
...
async function performAuthenticatedRequest(endpoint: string, token: string, options: any = {}) {
  return await runScript('authenticated-request', async () => {
...
async function performHealthCheck() {
  return await runScript('health-check', async () => {
...
async function performConcurrentRequests(endpoint: string, count: number) {
  return await runScript('concurrent-requests', async () => {
    const requests = [];
...
beforeAll(async () => {
  try {
    ...
    if (!validation.isValid) {
      throw createError('Test configuration validation failed', {
...
describe('🚀 Fire22 Enhanced API Integration Tests', () => {
  ...
  describe('🏥 Health Endpoints', () => {
    ...
    test('GET /api/health/status - should return detailed status information', async () => {
      const result = await runScript('health-status-check', async () => {
  ...
  describe('🔐 Authentication Endpoints', () => {
    test('POST /api/auth/login - should reject without credentials', async () => {
      const result = await runScript('login-no-credentials', async () => {
    ...
    test('POST /api/auth/login - should accept valid credentials', async () => {
      const result = await runScript('login-valid-credentials', async () => {
        const response = await fetch(`${testConfig.baseURL}/api/auth/login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    test('GET /api/auth/verify - should reject without token', async () => {
      const result = await runScript('verify-no-token', async () => {
  ...
  describe('👨‍💼 Manager Endpoints (Protected)', () => {
    ...
    beforeAll(async () => {
      const result = await runScript('manager-auth-setup', async () => {
    ...
    test('GET /api/manager/getLiveWagers - should require authentication', async () => {
      const result = await runScript('manager-unauthorized', async () => {
  ...
  describe('🛡️ Error Handling', () => {
    test('should return 404 for unknown endpoints', async () => {
      const result = await runScript('unknown-endpoint', async () => {
    ...
    test('should handle malformed JSON gracefully', async () => {
      const result = await runScript('malformed-json', async () => {
        const response = await fetch(`${testConfig.baseURL}/api/auth/login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: 'invalid json'
...
describe('⚡ Enhanced Performance Tests', () => {
  ...
  test('concurrent requests handling with enhanced monitoring', async () => {
    ...
    console.log(`   Success rate: ${(result.data.successCount / testConfig.concurrentRequests * 100).toFixed(1)}%`);
  });
...
describe('🔗 Enhanced Integration Tests', () => {
  test('full authentication flow with performance tracking', async () => {
    const result = await runScript('full-auth-flow', async () => {
      // 1. Login
...
describe('📊 Enhanced Test Reporting', () => {
  test('should generate comprehensive performance report', async () => {
    const result = await runScript('performance-report-generation', async () => {
