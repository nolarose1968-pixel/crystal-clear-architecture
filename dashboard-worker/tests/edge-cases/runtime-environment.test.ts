  ...
  describe("🧠 Memory Pressure Scenarios", () => {
    ...
    it("🧪 should work with memory constraints (--smol simulation)", async () => {
      const constrainedOperation = async () => {
        ...
        return smallData.reduce((acc, item) => ({
          count: acc.count + 1,
    ...
    it("🧪 should detect memory leaks", async () => {
      ...
      console.log(`📊 Memory usage: initial=${(initialMemory/1024/1024).toFixed(1)}MB, peak=${(midMemory/1024/1024).toFixed(1)}MB, final=${(finalMemory/1024/1024).toFixed(1)}MB`);
