...
export class BunVersionManager {
   ...
   */
  increment(type: "major" | "minor" | "patch" | "prerelease" = "patch", prereleaseId?: string): string {
    ...
    switch (type) {
      ...
      case "prerelease":
        if (current.prerelease && current.prerelease.length > 0) {
          const prereleaseVersion = typeof current.prerelease[1] === 'number' 
            ? current.prerelease[1] + 1 
            : 1;
   ...
   */
  private recordVersion(version: string, metadata: {
    author: string;
    changes: string[];
    breaking: boolean;
    gitTag?: string;
  ...
  getVersionsInRange(range: string): string[] {
    const query = this.versionDB.query("SELECT version FROM version_history ORDER BY timestamp DESC");
