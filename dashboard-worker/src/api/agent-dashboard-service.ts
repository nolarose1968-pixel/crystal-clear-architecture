    ...
    try {
      ...
      try {
        ...
        if (accountInfo) {
          ...
          result.financialPerformance.accountSummary.availableBalance = accountInfo.availableBalance || 0;
        }
      ...
      try {
        ...
        if (customersResponse && customersResponse.customers) {
          ...
          result.customerData.activeCustomers = result.customerData.customers.filter(c => c.active).length;
          result.customerData.suspendedCustomers = result.customerData.customers.filter(c => c.suspendSportsbook).length;
      ...
      try {
        ...
        if (wagersResponse && wagersResponse.wagers) {
          ...
          result.pendingWagers.totalPendingAmount = result.pendingWagers.wagers.reduce((sum, w) => sum + w.wagerAmount, 0);
          result.pendingWagers.totalPendingPayout = result.pendingWagers.wagers.reduce((sum, w) => sum + w.potentialPayout, 0);
      ...
      try {
        ...
        if (transactionsResponse && transactionsResponse.transactions) {
          result.recentTransactions.transactions = transactionsResponse.transactions.map((txn: any) => ({
          ...
          result.recentTransactions.totalTransactions = result.recentTransactions.transactions.length;
          result.recentTransactions.lastTransactionTime = result.recentTransactions.transactions.length > 0 
        ...
        if (newUsersResponse) {
          result.newUsersInfo = {
            ...
            newUsers: (newUsersResponse.newUsers || []).map((user: any) => ({
              ...
              registrationDate: user.registrationDate || user.createdAt || user.openDateTime || new Date().toISOString(),
              status: user.status || (user.active === false ? 'suspended' : user.active === true ? 'active' : 'pending'),
              initialDeposit: parseFloat(user.initialDeposit || user.firstDeposit || user.deposit || '0'),
      ...
      try {
        ...
        if (teaserResponse) {
          result.teaserProfile = {
            ...
            profile: {
              profileID: teaserResponse.teaserProfile?.profileID || teaserResponse.teaserProfile?.id || 'N/A',
              profileName: teaserResponse.teaserProfile?.profileName || teaserResponse.teaserProfile?.name || 'Default',
              description: teaserResponse.teaserProfile?.description || 'Teaser betting profile configuration',
              sports: teaserResponse.teaserProfile?.sports || [],
              pointSpreads: (teaserResponse.teaserProfile?.pointSpreads || []).map((spread: any) => ({
              ...
              totalLimits: (teaserResponse.teaserProfile?.limits || teaserResponse.limits || []).map((limit: any) => ({
              ...
              configuration: teaserResponse.teaserProfile?.configuration || teaserResponse.data || {}
            },
            settings: {
              allowTeasers: teaserResponse.settings?.allowTeasers !== false && teaserResponse.settings?.enabled !== false,
              maxTeams: parseInt(teaserResponse.settings?.maxTeams || teaserResponse.settings?.maximumTeams || '0'),
              minTeams: parseInt(teaserResponse.settings?.minTeams || teaserResponse.settings?.minimumTeams || '2'),
              allowTies: teaserResponse.settings?.allowTies !== false && teaserResponse.settings?.ties !== false,
              pushAction: teaserResponse.settings?.pushAction || teaserResponse.settings?.push || 'loss'
            },
