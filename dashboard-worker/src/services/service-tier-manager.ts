...
export class ServiceTierManager {
  ...
  public async processTierChange(
    ...
    const historyEntry: TierHistoryEntry = {
      ...
      toTier: newTier,
      changeDate: new Date(),
      changeReason: newTier > oldTier ? 'Automatic upgrade based on activity' : 'Automatic downgrade due to inactivity',
      triggerMetrics: {
    ...
    if (newTier > oldTier && TIER_UPGRADE_MESSAGES[newTier]) {
      notification = {
        ...
        telegramMessage: TIER_UPGRADE_MESSAGES[newTier].telegramMessage.replace('{customerName}', customer.username),
  ...
  public calculateNextTierProgress(
    ...
    const volumeProgress = Math.min(100, 
      (currentPeriod.lifetimeVolume / requirements.minLifetimeVolume) * 100);
    
    const transactionProgress = Math.min(100,
    ...
    if (missingRequirements.length === 0) {
      ...
      if (monthsNeeded < 12) { // Only estimate if less than a year
  ...
  public needsTierReview(serviceProfile: CustomerServiceProfile): boolean {
    const daysSinceLastReview = Math.floor(
      (Date.now() - serviceProfile.currentPeriod.lastTierReview.getTime()) / 
      (1000 * 60 * 60 * 24)
    );
   ...
   */
  public generateTelegramTierNotification(
    notification: TierUpgradeNotification
  ): string {
