...
export class HierarchyAggregator {
  ...
  private buildNaturalAggregation(): void {
    ...
    this.aggregated = {
      ...
      organizationalView: Array.from(nodes.values()).filter(n => n.sourceSystem === 'organizational'),
      ...
      sources: [
        { system: 'fire22', lastSync: new Date().toISOString(), nodeCount: this.countBySource(nodes, 'fire22') },
        { system: 'organizational', lastSync: new Date().toISOString(), nodeCount: this.countBySource(nodes, 'organizational') },
        { system: 'departments', lastSync: new Date().toISOString(), nodeCount: this.countBySource(nodes, 'department') }
      ]
    };
  }
  ...
  private processOrganizationalSystem(nodes: Map<string, AggregatedHierarchyNode>): void {
    ...
    const orgData = [
      { id: 'ceo_001', name: 'Michael Johnson', title: 'CEO', department: 'Executive', parentId: null },
      { id: 'cmo_001', name: 'Sarah Johnson', title: 'Chief Marketing Officer', department: 'Marketing', parentId: 'ceo_001' },
      { id: 'cto_001', name: 'Chris Brown', title: 'Chief Technology Officer', department: 'Technology', parentId: 'ceo_001' }
    ];
  ...
  query(options: {
    ...
    if (options.name) {
      results = results.filter(n => 
        n.name.toLowerCase().includes(options.name!.toLowerCase())
      );
    }
  ...
  private async loadFire22Hierarchy(): Promise<any[]> {
    ...
    return [
      { agentId: 'A001', name: 'Master Agent One', level: 1, department: 'Sales', parentId: null, children: ['A002'] },
      { agentId: 'A002', name: 'Senior Agent Two', level: 3, department: 'Sales', parentId: 'A001', children: [] }
    ];
  }
  ...
  private loadDepartmentData(department: string): any[] {
    ...
    const mockData: Record<string, any[]> = {
      marketing: [
        { id: 'mar001', name: 'Sarah Johnson', role: 'Marketing Director', level: 2, metrics: { implementations: 62 } },
        { id: 'mar002', name: 'Michelle Rodriguez', role: 'Marketing Manager', level: 3, reportsTo: 'mar001', metrics: { implementations: 45 } }
      ],
      operations: [
        { id: 'ops001', name: 'Jennifer Wilson', role: 'VP of Operations', level: 2, metrics: { implementations: 71 } },
        { id: 'ops002', name: 'David Martinez', role: 'Operations Director', level: 3, reportsTo: 'ops001', metrics: { implementations: 67 } }
      ]
    };
  ...
  private groupByDepartment(nodes: Map<string, AggregatedHierarchyNode>): Record<string, AggregatedHierarchyNode[]> {
  ...
  private findDepartmentConnections(nodes: AggregatedHierarchyNode[], crossReferences: any[]): void {
    ...
    for (const orgNode of nodes.filter(n => n.sourceSystem === 'organizational')) {
      for (const deptNode of nodes.filter(n => n.sourceSystem === 'department' && n.department === orgNode.department)) {
