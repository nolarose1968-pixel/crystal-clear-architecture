...
export class CustomerService {
  ...
  static validateTransaction(
    ...
    if (amount > maxAmount) {
      errors.push(`Transaction amount exceeds ${transactionType} limit of $${maxAmount.toLocaleString()}`);
    }
    ...
    if (amount > paymentConfig.maxAmount) {
      errors.push(`Amount exceeds maximum of $${paymentConfig.maxAmount.toLocaleString()} for ${paymentConfig.name}`);
    }
  ...
  static calculateCommission(
    ...
    const totalRate = Math.min(
      baseRate + tierBoost,
      SYSTEM_CONSTANTS.MAX_COMMISSION_PERCENTAGE
    );
    
    const commissionAmount = Math.max(
      amount * totalRate,
      SYSTEM_CONSTANTS.MIN_COMMISSION_AMOUNT
    );
