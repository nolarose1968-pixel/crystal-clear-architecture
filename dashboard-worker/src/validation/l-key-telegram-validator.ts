...
export class LKeyTelegramValidator {
  ...
  async validateLKeyTelegramConsistency(agentID?: string): Promise<ValidationReport> {
    ...
    try {
      ...
      try {
        ...
        const validationError = createValidationError(
          ValidationErrorCode.CUSTOMER_DATA_INVALID,
          { error: error.message, customerCount: customers.length }
        );
        this.errors.push(validationError);
      ...
      const fatalError = createValidationError(
   ...
   */
  private async validateCustomersPartial(customers: Fire22Customer[]): Promise<CustomerValidation[]> {
   ...
   */
  private async validateTelegramUsersPartial(customers: Fire22Customer[]): Promise<TelegramUserValidation[]> {
    ...
    for (const telegramUser of telegramUsers) {
      try {
        ...
        errorCount++;
        this.warnings.push(`Failed to validate Telegram user ${telegramUser.telegramId}: ${error.message}`);
  ...
  private async validateSingleTelegramUser(telegramUser: any): Promise<TelegramUserValidation> {
    try {
      ...
      if (telegramUser.username && !this.validateUsernameToIdConsistency(
   ...
   */
  private async validateTelegramUsers(customers: Fire22Customer[]): Promise<TelegramUserValidation[]> {
    const validations: TelegramUserValidation[] = [];
  ...
  private generateRecommendations(
    ...
    if (invalidTelegramIds > 0) {
      recommendations.push(
        `🔧 Fix ${invalidTelegramIds} invalid Telegram ID formats`
      );
    }
    
    if (missingUsernames > 0) {
      recommendations.push(
        `🔧 Add username or Telegram ID for ${missingUsernames} customers`
      );
    }
    
    recommendations.push(
      '📊 Run this validation periodically to maintain data consistency'
    );
    
    recommendations.push(
      '🔄 Consider implementing real-time validation on customer creation'
    );
  ...
  private countFixableIssues(
    ...
    const fixableCustomerIssues = customerValidations.filter(c =>
      c.issues.some(issue => 
        issue.includes('No L-key found') || 
        issue.includes('Invalid username format')
      )
    ).length;
    
    const fixableTelegramIssues = telegramValidations.filter(t =>
      t.issues.some(issue => 
        issue.includes('Missing or invalid first name')
      )
    ).length;
  ...
  private countCriticalIssues(
    ...
    const criticalCustomerIssues = customerValidations.filter(c =>
      c.issues.some(issue => 
        issue.includes('Missing both username') ||
        issue.includes('Invalid Telegram ID format')
      )
    ).length;
    ...
    const criticalTelegramIssues = telegramValidations.filter(t =>
      t.issues.some(issue => 
        issue.includes('Invalid Telegram ID format') ||
        issue.includes('Username does not match')
      )
    ).length;
  ...
  generateSummary(report: ValidationReport): string {
    const successRate = (report.validMappings / (report.totalCustomers + report.totalTelegramUsers) * 100).toFixed(1);
