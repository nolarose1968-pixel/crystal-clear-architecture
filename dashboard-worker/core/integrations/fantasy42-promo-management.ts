...
export class Fantasy42PromoManagement {
   ...
   */
  private async validatePromoSubmission(form: HTMLFormElement, promoType: string): Promise<boolean> {
    ...
    if (this.config.validationRules.riskAssessment) {
      ...
      if (riskScore > 0.7) {
        this.showFormError(form, 'Player risk assessment indicates high risk. Manual review required.');
        return false;
      }
    }
  ...
  private async submitPromoAllocation(form: HTMLFormElement, promoType: string): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showSuccessMessage(`Promo ${this.config.promoTypes[promoType].name} allocated successfully!`);
   ...
   */
  private async validateBusinessRules(playerId: string, promoType: string, amount: number): Promise<boolean> {
    ...
    if (dailyAllocated + amount > this.config.allocationRules.dailyLimit) {
      this.showFormError(document.querySelector('form[data-promo-form]') as HTMLFormElement,
  ...
  private setFormLoading(form: HTMLFormElement, loading: boolean): void {
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent = loading ? 'Processing...' : (submitBtn.textContent?.replace('Processing...', 'Allocate Promo') || 'Allocate Promo');
    }
  }
   ...
   */
  async allocatePromoToPlayer(playerId: string, promoType: string, amount: number, metadata?: Record<string, any>): Promise<boolean> {
    try {
      const allocationData = {
        playerId,
        promoType,
