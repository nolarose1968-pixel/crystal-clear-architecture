...
export class Fantasy42CustomerInfo {
   ...
   */
  private async handleFieldChange(fieldName: string, element: Element, type: string): Promise<void> {
    let value: any = '';
  ...
  private updateFieldValidationStatus(fieldName: string, isValid: boolean): void {
    ...
    this.currentCustomer.validationStatus[fieldName as keyof typeof this.currentCustomer.validationStatus] = isValid;
  ...
  private async handleTelegramToggle(event: Event): Promise<void> {
    ...
    if (this.currentCustomer) {
      ...
      // Update Fantasy42 backend
      await this.fantasyClient.updateCustomerTelegramSetting(this.currentCustomer.customerId, enabled);
    }
  }
  ...
  private async getCurrentCustomerId(): Promise<string | null> {
    ...
    const customerIdField = document.querySelector('input[data-field="customerId"], input[name="customerId"]') as HTMLInputElement;
