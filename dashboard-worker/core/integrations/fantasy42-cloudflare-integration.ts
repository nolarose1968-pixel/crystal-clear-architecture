...
export class Fantasy42CloudflareIntegration {
	...
	this.analytics.challengeMetrics.averageCompletionTime =
	  (this.analytics.challengeMetrics.averageCompletionTime *
	   (this.analytics.challengeMetrics.successfulChallenges - 1) + duration) /
	  this.analytics.challengeMetrics.successfulChallenges;
	...
	if (this.challengeState.attempts < this.config.challengeRetryAttempts) {
	  console.log(`🔄 Retrying challenge: ${challengeId} (attempt ${this.challengeState.attempts + 1})`);
	...
	console.log(`🔄 Challenge retry initiated: ${challengeId} (attempt ${this.challengeState.attempts})`);
	}
	...
	if (this.config.securityHeaders.contentSecurityPolicy) {
	  securityHeaders.push({
	    ...
	    content: "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn-cgi.challenge-platform.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://api.fantasy42.com;"
	  });
	}
	...
	const rejectionHandler = (event: PromiseRejectionEvent) => {
	  if (event.reason?.message?.includes('Cloudflare') || event.reason?.stack?.includes('jsd/main.js')) {
	    this.analytics.errorMetrics.scriptErrors++;
...
export const initializeFantasy42CloudflareIntegration = async (
	...
	const cfIntegration = new Fantasy42CloudflareIntegration(fantasyClient, unifiedIntegration, config);
