...
import { XPathElementHandler, handleFantasy402Element, findFantasy402Element } from '../ui/xpath-element-handler';
...
export class Fantasy402NotesManager {
  ...
  constructor(client: Fantasy402AgentClient) {
    ...
    this.elementPath = '/html/body/div[3]/div[5]/div/div[5]/section/section/div/div/div/div[2]/div/div';
  }
   ...
   */
  private determineNotesCategory(notes: string): 'general' | 'complaint' | 'praise' | 'warning' | 'suspension' | 'vip' | 'risk' | 'lottery' {
    const lowerNotes = notes.toLowerCase();

    if (lowerNotes.includes('complaint') || lowerNotes.includes('unhappy') || lowerNotes.includes('problem')) {
      return 'complaint';
    }

    if (lowerNotes.includes('excellent') || lowerNotes.includes('great') || lowerNotes.includes('wonderful') || lowerNotes.includes('amazing')) {
      return 'praise';
    }

    if (lowerNotes.includes('warning') || lowerNotes.includes('caution') || lowerNotes.includes('attention') || lowerNotes.includes('monitor')) {
      return 'warning';
    }

    if (lowerNotes.includes('suspend') || lowerNotes.includes('suspension') || lowerNotes.includes('blocked')) {
      return 'suspension';
    }

    if (lowerNotes.includes('vip') || lowerNotes.includes('premium') || lowerNotes.includes('gold') || lowerNotes.includes('platinum')) {
      return 'vip';
    }

    if (lowerNotes.includes('risk') || lowerNotes.includes('fraud') || lowerNotes.includes('suspicious')) {
      return 'risk';
    }

    if (lowerNotes.includes('lottery') || lowerNotes.includes('lotto') || lowerNotes.includes('powerball') || lowerNotes.includes('mega')) {
      return 'lottery';
    }
  ...
  private extractCustomerIdFromPage(): string | null {
    ...
    const customerId = urlParams.get('customerId') || urlParams.get('playerId') || urlParams.get('id');
    if (customerId) return customerId;

    // 3. Look for customer ID in form fields
    const customerIdField = document.querySelector('input[name="customerId"], input[name="playerId"], input[name="customer_id"]') as HTMLInputElement;
    ...
    for (const heading of headings) {
      ...
      const match = text.match(/Customer[:\s]+([A-Z0-9]+)/i) || text.match(/Player[:\s]+([A-Z0-9]+)/i);
  ...
  private showValidationStatus(validation: { isValid: boolean; errors: string[] }): void {
    ...
    if (errorContainer) {
      if (validation.errors.length > 0) {
        errorContainer.innerHTML = validation.errors.map(error => `<div class="text-danger">${error}</div>`).join('');
...
export const createFantasy402NotesManager = (client: Fantasy402AgentClient): Fantasy402NotesManager => {
  return new Fantasy402NotesManager(client);
};

export const initializeFantasy402Notes = async (client: Fantasy402AgentClient): Promise<boolean> => {
