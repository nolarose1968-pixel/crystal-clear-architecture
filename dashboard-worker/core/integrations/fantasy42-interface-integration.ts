...
export class Fantasy42InterfaceIntegration {
	...
	console.log('✅ Keyboard shortcuts setup: Ctrl+Shift+P (Process), Ctrl+Shift+A (Toggle), Ctrl+Shift+S (Status)');
	}
	...
	if (this.automation) {
	  ...
	  if (matches.length > 0) {
	    ...
	    alert(`No P2P matches found for ${thirdPartyId}. The system will continue monitoring for new matches.`);
	  }
	}
	}
	...
	const agentSelect = document.querySelector('select[data-field="agent-parent"]') as HTMLSelectElement;
	...
	if (indicator) {
	  ...
	  if (statusText) {
	    statusText.textContent = status === 'active' ? 'P2P Automation Active' : 'P2P Automation Inactive';
	  }

	  // Update background color
	  indicator.style.background = status === 'active'
	    ? 'rgba(40, 167, 69, 0.9)'
	    : 'rgba(220, 53, 69, 0.9)';
	}
	}
