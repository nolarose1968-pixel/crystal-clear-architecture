...
export class Fantasy42EnhancedModals {
	...
	const container = button.closest('.data-visualization-controls')?.querySelector('.visualization-container') as HTMLElement;
	if (container) {
	...
	return {
	  ...
	  aiFeaturesActive: Array.from(this.modalIntelligence.values()).filter(m => m.features.aiSuggestions).length,
...
export const initializeFantasy42EnhancedModals = async (
	...
	const enhancedModals = new Fantasy42EnhancedModals(modalsIntegration, fantasyClient, unifiedIntegration);
