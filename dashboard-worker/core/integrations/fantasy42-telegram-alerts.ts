...
export class Fantasy42TelegramAlerts {
	...
	if (label.htmlFor) {
	  ...
	  if (container) {
	    const inputs = container.querySelectorAll('input[type="checkbox"], input[type="radio"], button');
	...
	if (lastBet && (now.getTime() - lastBet.getTime()) < 60000) { // Less than 1 minute
	  return true;
	}
...
Please review and take appropriate action.
	...
	if (alertElement) {
	  ...
	  const originalText = alertElement.textContent?.replace(' (Enabled)', '').replace(' (Disabled)', '') || '';
...
export const createFantasy42TelegramAlerts = (
	...
	return new Fantasy42TelegramAlerts(fantasyClient, telegramBot, signalIntegration, wagerAnalysis, config);
};
...
export const initializeFantasy42TelegramAlerts = async (
	...
	const alerts = new Fantasy42TelegramAlerts(fantasyClient, telegramBot, signalIntegration, wagerAnalysis, config);
