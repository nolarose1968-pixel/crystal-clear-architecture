...
export class Fantasy42TelegramUserId {
	...
	const sendVerificationBtn = container.querySelector('#send-verification-btn') as HTMLButtonElement;
	 ...
	 * Show status message
	 */
	private showStatusMessage(message: string, type: 'success' | 'error' | 'pending' = 'pending'): void {
	...
	buttons.forEach(button => {
	  ...
	  button.textContent = loading ? 'Processing...' : button.textContent?.replace('Processing...', '') || 'Process';
	});
	}
	...
	if (verifiedElement) verifiedElement.textContent = this.analytics.users.verifiedUsers.toString();
	if (pendingElement) pendingElement.textContent = this.analytics.users.pendingUsers.toString();
	if (successElement) successElement.textContent = `${Math.round(this.analytics.verification.successRate * 100)}%`;
	}
	...
	this.analytics.verification.successRate =
	  ...
	  this.analytics.verification.successfulVerifications / this.analytics.verification.totalAttempts : 0;
	...
	return {
	  ...
	  verifiedUsers: Array.from(this.telegramUsers.values()).filter(u => u.verificationStatus === 'verified').length,
