...
export interface PaymentMethod {
	id: string;
	type: 'credit_card' | 'debit_card' | 'bank_transfer' | 'paypal' | 'crypto' | 'venmo' | 'cashapp' | 'zelle';
	name: string;
	maskedNumber: string;
	expiryDate?: string;
	isDefault: boolean;
...
export class Fantasy42Cashier {
	...
	if (submitBtn) {
	  ...
	  submitBtn.textContent = loading ? 'Processing...' : (submitBtn.textContent?.replace('Processing...', '') || 'Submit');
	}
	}
