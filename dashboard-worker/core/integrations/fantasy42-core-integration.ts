...
export class Fantasy42CoreIntegration {
	 ...
	 * Handle view change
	 */
	private async handleViewChange(view: 'mobile' | 'desktop' | 'bet-ticker', reason: string): Promise<void> {
	...
	if (menuToggle) {
	  const toggleListener = () => {
	    ...
	    // Update session data
	    this.sessionData.featureUsage.menu_toggle = (this.sessionData.featureUsage.menu_toggle || 0) + 1;
	 ...
	 * Force view change
	 */
	async forceViewChange(view: 'mobile' | 'desktop' | 'bet-ticker', reason: string = 'manual'): Promise<void> {
