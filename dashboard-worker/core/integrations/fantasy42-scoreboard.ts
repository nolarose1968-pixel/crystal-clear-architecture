...
export class Fantasy42Scoreboard {
	...
	document.getElementById('export-json')?.addEventListener('click', () => this.exportGames('json'));
	...
	const gameContainer = scoreboard.querySelector('.game-container') || this.createGameContainer(scoreboard);
	  ...
	  <div class="game-card ${statusClass}" data-game-id="${game.gameId}">
	    ...
	    ${game.significantPlays.length > 0 ? `
	      <div class="significant-plays">
	        ${game.significantPlays.slice(-3).map(play => `
	...
	return Array.from(this.liveGames.values()).filter(game => {
	  ...
	  if (searchTerm) {
	    const searchableText = `${game.homeTeam} ${game.awayTeam} ${game.sport} ${game.league}`.toLowerCase();
	...
	const headers = ['Game ID', 'Sport', 'League', 'Home Team', 'Away Team', 'Home Score', 'Away Score', 'Status', 'Period', 'Time Remaining'];
