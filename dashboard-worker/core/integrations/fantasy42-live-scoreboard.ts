...
export class Fantasy42LiveScoreboard {
	...
	const winProbability = Math.round((baseProbability > 0.5 ? baseProbability : 1 - baseProbability) * 100);
	...
	for (const game of liveGames.slice(0, 3)) { // Show top 3 games
	...
	const riskLevel: 'low' | 'medium' | 'high' = expectedValue > 0.1 ? 'low' : expectedValue > 0 ? 'medium' : 'high';

	if (expectedValue > 0.05) { // Only suggest if EV > 5%
	  return {
	    gameId: game.gameId,
	    betType: 'spread',
...
export const initializeFantasy42LiveScoreboard = async (
	...
	const liveScoreboard = new Fantasy42LiveScoreboard(scoreboard, fantasyClient, unifiedIntegration, features);
