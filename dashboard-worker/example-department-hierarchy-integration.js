...
async function loadFire22AgentsView() {
    ...
    */
    ...
    return {
        ...
        masterAgents: fire22Data.results.filter(agent => 
            agent.sourceData?.level === 1
        ),
        clerks: fire22Data.results.filter(agent => 
            agent.sourceData?.level === 8
        )
    };
}
...
async function analyzeOrganizationalConnections() {
    ...
    const analysis = {
        ...
        highConfidenceMatches: crossRefs.crossReferences.filter(ref => 
            ref.confidence > 0.9 && ref.likely_same_person
        ),
        
        potentialDuplicates: crossRefs.crossReferences.filter(ref =>
            ref.connections.length > 1 && ref.likely_same_person
        ),
