...
async function runAuditTrailDemo(): Promise<void> {
  ...
  for (const change of balanceChanges) {
    ...
    try {
      ...
      console.log(`  ✅ Logged ${change.type} for ${change.customerId}: $${change.amount.toLocaleString()}`);
    } catch (error) {
  ...
  try {
    ...
    recentChanges.slice(0, 5).forEach(change => {
      ...
      console.log(`  ${change.timestamp}: ${change.customerId} - ${sign}$${change.changeAmount.toLocaleString()} (${change.changeType})`);
    });
  } catch (error) {
...
async function runNotificationsDemo(): Promise<void> {
  ...
  const testScenarios = [
    { customerId: 'DEMO001', currentBalance: 500, previousBalance: 5000, description: 'Large balance drop' },
    { customerId: 'DEMO002', currentBalance: 50, previousBalance: 1000, description: 'Critical threshold' },
    { customerId: 'DEMO003', currentBalance: 800, previousBalance: 1500, description: 'Warning threshold' }
  ];
...
async function runAnalyticsDemo(): Promise<void> {
  ...
  for (const customer of DEMO_CUSTOMERS) {
    ...
    try {
      ...
      console.log(`  📅 Period: ${analytics.period} (${analytics.startDate} to ${analytics.endDate})`);
      ...
      console.log(`  📈 Net Change: $${analytics.netChange.toLocaleString()} (${analytics.changePercentage.toFixed(1)}%)`);
  ...
  try {
    const systemAnalytics = await BalanceAnalyticsService.generateSystemAnalytics(DEMO_AGENT, 'daily');
