...
class ComprehensiveTestRunner {
  ...
  private async runTestSuite(suite: TestSuite): Promise<void> {
    ...
    try {
      ...
      const testPath = suite.pattern ? `${suite.path}/${suite.pattern}` : suite.path;

      // Run tests with coverage if available
      const proc = Bun.spawn(['/opt/homebrew/bin/bun', 'test', testPath, '--bail', '--coverage'], {
        stdout: 'pipe',
        stderr: 'pipe',
        env: {
          ...process.env,
      ...
      if (exitCode === 0) {
        ...
        console.log(
          `  ✅ ${suite.name}: ${testStats.tests} tests passed (${(elapsed / 1000).toFixed(2)}s)`
        );
      } else {
        ...
        console.log(
          `  ❌ ${suite.name}: ${testStats.failures} failures (${(elapsed / 1000).toFixed(2)}s)`
        );
