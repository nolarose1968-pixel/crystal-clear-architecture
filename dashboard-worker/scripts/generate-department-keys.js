...
function generateDepartmentCredentials(department, index) {
  const credentials = {
    ...
    ssh: {
      ...
      key_type: "ed25519",
      repository_access: department.security_clearance === "level_4" ? "all_repositories" : "department_specific",
      expiration: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    },
    ...
    database: {
      ...
      schema_access: department.security_clearance === "level_4" ? "all_schemas" : `${department.id}_schema`,
      permissions: {
        read: true,
...
async function generateAllCredentials() {
  ...
  for (let i = 0; i < DEPARTMENTS.length; i++) {
    ...
    await writeFile(departmentFile, JSON.stringify({
      department: department,
