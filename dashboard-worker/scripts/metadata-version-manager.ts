...
export class MetadataVersionManager {
   ...
   */
  async bumpVersions(strategy: VersionStrategy, options: {
    commit?: boolean;
    tag?: boolean;
    push?: boolean;
    verbose?: boolean;
  ...
  async generateChangelog(since?: string): Promise<string> {
    ...
    changes.push('');
    changes.push(`- **Build Constants**: ${Object.keys(this.workspace.buildConstants).length} constants`);
   ...
   */
  async buildWorkspaceWithVersions(options: {
    packages?: string[];
  ...
  generateVersionReport(): string {
    ...
    const sortedPackages = Array.from(this.workspace.packages.entries())
    ...
    for (const [name, pkg] of sortedPackages) {
      ...
      if (pkg.metadata?.responsibilities && pkg.metadata.responsibilities.length > 0) {
        report.push(`  - Responsibilities: ${pkg.metadata.responsibilities.slice(0, 2).join(', ')}${pkg.metadata.responsibilities.length > 2 ? '...' : ''}`);
      }
    ...
    report.push(`- Package Versions: ${Object.keys(this.workspace.buildConstants.PACKAGE_VERSIONS || {}).length}`);
    report.push(`- Components: ${(this.workspace.buildConstants.WORKSPACE_COMPONENTS || []).length}`);
  ...
  private async calculateNewVersion(pkg: PackageMetadata, strategy: VersionStrategy): Promise<string> {
  ...
  private async buildPackage(packageName: string, options: {
    mode: 'development' | 'production';
    ...
    try {
      ...
      if (result.success) {
        ...
        return { success: false, output: result.output || '', skipped: false, reason: result.error };
      }
    } catch (error) {
