...
import { readFileSync, writeFileSync, existsSync, statSync, readdirSync, createReadStream, createWriteStream } from 'fs';
...
export class FileOperations {
   ...
   */
  static async readFile(filePath: string, options: { streaming?: boolean; maxSize?: number } = {}): Promise<string> {
   ...
   */
  static async writeFile(filePath: string, content: string, options: { atomic?: boolean; backup?: boolean } = {}): Promise<void> {
   ...
   */
  static async getFileHash(filePath: string, algorithm: 'md5' | 'sha1' | 'sha256' = 'sha256'): Promise<string> {
    if (!existsSync(filePath)) {
...
export class OptimizedMap<K, V> {
  ...
  constructor(maxSize: number = 10000, ttl: number = 5 * 60 * 1000) { // 5 minutes default TTL
    this.maxSize = maxSize;
    this.ttl = ttl;
  }
