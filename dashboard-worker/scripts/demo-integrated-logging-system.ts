...
import { OTCMatchingEngine, OrderType, OrderSide, TradingAsset } from '../src/services/otc-order-matching-engine';
import {
  CustomerType,
  TransactionType,
  PaymentMethod,
...
async function demonstrateOTCTradingWithLogging(logger: any): Promise<void> {
  ...
  console.log(`   Order: ${blockOrder.side} $${blockOrder.amount.toLocaleString()} of ${blockOrder.asset}`);
  ...
  try {
    ...
    logger.error('OTC order placement failed', {
...
async function demonstrateReporting(logger: any): Promise<void> {
  ...
  console.log('\n   ⚡ Performance Metrics:');
  console.log(`     Average Response Time: ${auditReport.performanceMetrics.averageResponseTime.toFixed(2)}ms`);
  ...
  console.log(`     Average Risk Score: ${auditReport.complianceMetrics.averageRiskScore.toFixed(1)}/100`);
  
  // Get L-Key usage statistics
  const lKeyStats = logger.getLKeyUsageStats();
  const totalUsage = Object.values(lKeyStats).reduce((sum: number, stat: any) => sum + stat.usage, 0);
