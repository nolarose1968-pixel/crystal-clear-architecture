...
class Fire22APIClient {
  ...
  constructor() {
    this.security = new Fire22SecurityManager(
      FIRE22_CONFIG.apiKey,
      FIRE22_CONFIG.apiSecret
    );
  ...
  async testCustomerEndpoints() {
    ...
    try {
      ...
      console.log(`✅ GET /api/customers - ${responseTime.toFixed(2)}ms - ${data?.length || 0} customers`);
      
      if (data && data.length > 0) {
        // Get specific customer
        const customerId = data[0].id;
        const { data: customer, responseTime: custTime } = await this.request('GET', `/api/customers/${customerId}`);
        console.log(`✅ GET /api/customers/${customerId} - ${custTime.toFixed(2)}ms`);
        
        // Get customer balance
        const { data: balance, responseTime: balTime } = await this.request('GET', `/api/customers/${customerId}/balance`);
        console.log(`✅ GET /api/customers/${customerId}/balance - ${balTime.toFixed(2)}ms - Balance: $${balance?.balance || 0}`);
      }
    } catch (error) {
  ...
  async testAgentEndpoints() {
    ...
    try {
      ...
      console.log(`✅ GET /api/agents/hierarchy - ${responseTime.toFixed(2)}ms - ${data?.length || 0} agents`);
      
      // Get agent stats
      const { data: stats, responseTime: statsTime } = await this.request('POST', '/api/manager/getWeeklyFigureByAgent', {
        agentId: 'demo-agent',
      ...
      const { data: perms, responseTime: permsTime } = await this.request('GET', '/api/agents/permissions');
  ...
  async testTransactionEndpoints() {
    ...
    try {
      ...
      console.log(`✅ GET /api/transactions - ${responseTime.toFixed(2)}ms - ${data?.length || 0} transactions`);
      
      // Get transaction summary
      const { data: summary, responseTime: summaryTime } = await this.request('GET', '/api/transactions/summary');
  ...
  async testWagerEndpoints() {
    ...
    try {
      ...
      console.log(`✅ POST /api/manager/getLiveWagers - ${responseTime.toFixed(2)}ms - ${data?.length || 0} wagers`);
      
      // Get wager statistics
      const { data: stats, responseTime: statsTime } = await this.request('GET', '/api/wagers/stats');
  ...
  async testFire22Endpoints() {
    ...
    try {
      ...
      // Test auth status
      const { data: authStatus, responseTime: authTime } = await this.request('POST', '/api/fire22/auth-status');
      console.log(`✅ POST /api/fire22/auth-status - ${authTime.toFixed(2)}ms - Authorized: ${authStatus?.authorized || false}`);
      
      // Test cache stats
      const { data: cache, responseTime: cacheTime } = await this.request('GET', '/api/fire22/cache-stats');
      console.log(`✅ GET /api/fire22/cache-stats - ${cacheTime.toFixed(2)}ms - Hit Rate: ${cache?.hitRate || 0}%`);
      
      // Test DNS stats
      const { data: dns, responseTime: dnsTime } = await this.request('GET', '/api/fire22/dns-stats');
      console.log(`✅ GET /api/fire22/dns-stats - ${dnsTime.toFixed(2)}ms - Cache Size: ${dns?.cacheSize || 0}`);
    } catch (error) {
  ...
  generateReport() {
    ...
    console.log(`Session Duration: ${((Date.now() - this.sessionStartTime.getTime()) / 1000).toFixed(2)}s`);
    ...
    testResults.forEach(result => {
      ...
      existing.count++;
      existing.avgTime = (existing.avgTime * (existing.count - 1) + result.responseTime) / existing.count;
    ...
    endpointStats.forEach((stats, endpoint) => {
      ...
      console.log(`    Calls: ${stats.count} | Avg Time: ${stats.avgTime.toFixed(2)}ms | Success: ${stats.successRate.toFixed(0)}%`);
    });
