...
class DataExtractionReadinessChecker {
  ...
  private async checkInfrastructure(): Promise<ReadinessReport['infrastructure']> {
    ...
    try {
      ...
      console.log(`   ${infrastructure.localSqlite ? '✅' : '❌'} Local SQLite: ${infrastructure.localSqlite}`);
    } catch (error) {
    ...
    try {
      ...
      infrastructure.cloudflareD1 = wranglerConfig.includes('d1_databases') && wranglerConfig.includes('fire22-dashboard');
      console.log(`   ${infrastructure.cloudflareD1 ? '✅' : '❌'} Cloudflare D1: ${infrastructure.cloudflareD1}`);
    } catch (error) {
    ...
    try {
      const wranglerConfig = await Bun.file('wrangler.toml').text();
      infrastructure.r2Storage = wranglerConfig.includes('r2_buckets') && wranglerConfig.includes('fire22-packages');
      console.log(`   ${infrastructure.r2Storage ? '✅' : '❌'} R2 Storage: ${infrastructure.r2Storage}`);
    } catch (error) {
    ...
    try {
      const wranglerConfig = await Bun.file('wrangler.toml').text();
      infrastructure.kvCaching = wranglerConfig.includes('kv_namespaces') && wranglerConfig.includes('FIRE22_DATA_CACHE');
      console.log(`   ${infrastructure.kvCaching ? '✅' : '❌'} KV Caching: ${infrastructure.kvCaching}`);
    } catch (error) {
  ...
  private async checkDataStatus(): Promise<ReadinessReport['dataStatus']> {
    ...
    try {
      ...
      const totalRecords = Object.values(dataStatus.currentRecords).reduce((sum, count) => sum + count, 0);
      
      // Estimate completion percentage (assuming full dataset is ~100k records)
      const estimatedFullDataset = 100000;
      dataStatus.estimatedCompletionPercent = Math.min((totalRecords / estimatedFullDataset) * 100, 100);
      ...
      console.log(`   📈 Estimated Completion: ${dataStatus.estimatedCompletionPercent.toFixed(1)}%`);
  ...
  private checkRetentionConfig(): ReadinessReport['retentionConfig'] {
    ...
    if (config.r2ArchiveYears > 1) {
      config.recommendedChanges.push(`Consider reducing R2 retention from ${config.r2ArchiveYears} years to 1 year as originally planned`);
  ...
  private checkTechnicalCapabilities(): ReadinessReport['technicalCapabilities'] {
    ...
    console.log(`   🔐 Secure Authentication: ${capabilities.secureAuth ? '✅' : '❌'} Bun.secrets`);
    console.log(`   🌐 DNS Optimization: ${capabilities.dnsOptimized ? '✅' : '❌'} Sub-ms resolution`);
    console.log(`   ⚡ Performance Ready: ${capabilities.performanceReady ? '✅' : '❌'} Caching & pooling`);
  ...
  private calculateReadinessScore(report: ReadinessReport): number {
    ...
    const totalRecords = Object.values(report.dataStatus.currentRecords).reduce((sum, count) => sum + count, 0);
  ...
  printSummaryReport(report: ReadinessReport): void {
    ...
    console.log(`   • Transactions: ${report.dataStatus.currentRecords.transactions.toLocaleString()}`);
    console.log(`   • Bets: ${report.dataStatus.currentRecords.bets.toLocaleString()}`);
    console.log(`   • Estimated Progress: ${report.dataStatus.estimatedCompletionPercent.toFixed(1)}%`);
    ...
    console.log(`   • Infrastructure: ${Object.values(report.infrastructure).filter(Boolean).length}/4 components`);
    console.log(`   • Security: ${report.technicalCapabilities.secureAuth ? 'Bun.secrets configured' : 'Needs setup'}`);
    console.log(`   • Performance: ${report.technicalCapabilities.performanceReady ? 'DNS + caching optimized' : 'Needs optimization'}`);
