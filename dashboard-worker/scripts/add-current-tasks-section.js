...
const departmentTasks = {
  ...
  'customer-support-department.html': {
    tasks: [
      ...
      {
        title: '24/7 Support Coverage Expansion',
        status: 'active',
        priority: 'high',
  ...
  'operations-department.html': {
    tasks: [
      ...
      {
        title: 'Facility Upgrade Planning',
        status: 'planning',
        priority: 'low',
        assignee: 'Mark Davis',
        progress: 15,
  ...
  'team-contributors-department.html': {
    tasks: [
      {
        title: 'Cross-Team Innovation Sprint',
        status: 'in-progress',
        priority: 'high',
        assignee: 'Brenda Williams',
        progress: 40,
      },
      {
  ...
  for (const [filename, data] of Object.entries(departmentTasks)) {
    ...
    try {
      ...
      const insertionPattern =
        /<\/ul>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<div class="department-overview">/;
      const recentContribPattern =
        /(<h3[^>]*>🚀 Recent Contributions<\/h3>[\s\S]*?<\/ul>\s*<\/div>)/;
      ...
      if (match) {
        ...
        content =
          content.slice(0, endOfContributions) +
          '\n' +
          tasksHTML +
