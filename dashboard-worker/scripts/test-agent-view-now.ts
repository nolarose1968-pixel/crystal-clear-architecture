...
async function testAgentViewNow() {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(
        `      Today P&L: $${data.data.financialPerformance?.currentWeek?.todayProfit?.toLocaleString()}`
      );
      console.log(
        `      Active Players: ${data.data.financialPerformance?.currentWeek?.activePlayers?.toLocaleString()}`
      );
      console.log(
        `      Account Balance: $${data.data.financialPerformance?.accountSummary?.balance?.toLocaleString()}`
      );
