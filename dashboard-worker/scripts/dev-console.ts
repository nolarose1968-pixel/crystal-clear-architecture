...
class Fire22DevConsole {
  ...
  private async setConsoleDepth(args: string[]): Promise<void> {
    if (args.length === 0) {
      ...
      console.log('📝 Note: This affects current session only. Update bunfig.toml for persistence.');
      return;
    }
  ...
  private async setWorkspace(args: string[]): Promise<void> {
    ...
    const fullName = workspaceName.startsWith('@fire22-') ? workspaceName : `@fire22-${workspaceName}`;
