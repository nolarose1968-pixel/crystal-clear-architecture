...
export class EnhancedBuildSystem {
   ...
   */
  async build(config: BuildConfig): Promise<{ success: boolean; defines: BuildDefines; outputPath: string }> {
    ...
    try {
      ...
      Logger.info(`Build command: ${buildCommand.slice(0, 2).join(' ')} [${buildCommand.length - 2} defines]`);
      ...
      Logger.performance(`Build completed`, perfResult.totalTime, perfResult.memoryUsage.heapUsed / 1024 / 1024);
...
export {};`;
  ...
  private async createBuildCommand(config: BuildConfig, defines: BuildDefines): Promise<string[]> {
    ...
    Object.entries(defines).forEach(([key, value]) => {
      const definedValue = typeof value === 'string' 
  ...
  private getApiUrl(target: string): string {
    switch (target) {
      ...
      case 'demo': return 'https://demo-api.fire22.com';
  ...
  private getApiTimeout(target: string): number {
    switch (target) {
      case 'development': return 10000; // 10 seconds
      case 'staging': return 20000; // 20 seconds
      case 'production': return 30000; // 30 seconds
  ...
  private getFire22ApiUrl(target: string): string {
    switch (target) {
      case 'development': return 'https://dev-fire22-api.example.com';
      case 'staging': return 'https://staging-fire22-api.example.com';
      case 'production': return 'https://fire22-api.example.com';
      default: return 'https://dev-fire22-api.example.com';
    }
  }
...
if (import.meta.main) {
  ...
  if (args.includes('--all')) {
    ...
    const config: BuildConfig = {
      ...
      entryPoint,
      outputFile: join(process.cwd(), `dist/fire22-${target}${process.platform === 'win32' ? '.exe' : ''}`),
      compile: true,
