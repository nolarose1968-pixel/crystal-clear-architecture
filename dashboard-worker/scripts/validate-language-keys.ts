...
class LanguageKeyValidator {
  ...
  private validateTranslations(): void {
    ...
    const languages = Object.keys(this.languageData.languages).filter(lang => !lang.startsWith('_'));
    const englishKeys = Object.keys(this.languageData.languages['en'] || {}).filter(key => !key.startsWith('_'));
    
    for (const lang of languages) {
      if (lang === 'en') continue; // Skip English as it's the base
      
      const langKeys = Object.keys(this.languageData.languages[lang] || {}).filter(key => !key.startsWith('_'));
  ...
  private validateKeyUsage(): void {
    ...
    undefinedKeys.forEach(key => {
      this.validationResult.errors.push(`Key '${key}' used in files but not defined in language data`);
    });
