...
class SideEffectsOptimizationDemo {
  ...
  private async analyzeBundleSizes(): Promise<BundleAnalysis[]> {
    ...
    for (const pkg of this.packages) {
      try {
        ...
        console.log(`   Saved:  ${this.formatBytes(reduction)} (${reductionPercent.toFixed(1)}%)\n`);
  ...
  private displayResults(bundleAnalysis: BundleAnalysis[], treeShakingMetrics: TreeShakingMetrics): void {
    ...
    console.log(`   Total Saved:  ${this.formatBytes(totalReduction)} (${totalReductionPercent.toFixed(1)}%)\n`);
  ...
  private simulateAfterSize(packageName: string): number {
    ...
    const reductionFactors: Record<string, number> = {
      ...
      'middleware': 0.80,       // 20% reduction
      'testing-framework': 0.75, // 25% reduction
      'wager-system': 0.60      // 40% reduction
    };
