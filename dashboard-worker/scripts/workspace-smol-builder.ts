...
export class WorkspaceSmolBuilder {
  ...
  async buildAllWorkspaces(): Promise<void> {
    ...
    Logger.info(`📦 Building ${Object.keys(this.config.workspaces).length} workspaces with SMOL optimization...`);
    Logger.info('='.repeat(60));
    ...
    for (const workspaceName of buildOrder) {
      ...
      if (result.success) {
        Logger.info(`✅ ${workspace.name}: ${this.formatBytes(result.size)} (${result.compressionRatio.toFixed(1)}% compressed)`);
      } else {
   ...
   */
  private async buildWorkspace(workspaceName: string, workspace: WorkspaceConfig): Promise<{
    name: string;
    success: boolean;
    size: number;
    compressionRatio: number;
   ...
   */
  private createWorkspaceSmolConfig(workspaceName: string, workspace: WorkspaceConfig): SmolBuildConfig {
  ...
  private generateBuildSummary(results: Array<any>, performance: any): void {
    ...
    Logger.info(`  📉 Size Reduction: ${sizeReduction.toFixed(1)}% (${this.formatBytes(this.totalSizeBefore - totalSizeAfter)} saved)`);
