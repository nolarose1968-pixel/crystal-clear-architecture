...
class TestBenchmarkRunner {
  ...
  private generateReport(analysis: BenchmarkAnalysis): void {
    ...
    console.log(
      `  Tests: ${current.testCount} (${current.passedCount} passed, ${current.failedCount} failed)`
    );
    if (current.coverage) {
    ...
    if (baseline) {
      ...
      console.log(
        `  Baseline: ${(baseline.duration / 1000).toFixed(2)}s (${baseline.timestamp.split('T')[0]})`
      );
    }
    ...
    const reportFile = join(
      this.benchmarkDir,
      `benchmark-${current.timestamp.split('T')[0]}-${current.gitCommit}.json`
    );
    try {
