...
class EndpointConsolidator {
  ...
  private extractEndpointsFromCloudflare(content: string, fileName: string): EndpointInfo[] {
    ...
    lines.forEach((line, index) => {
      ...
      const match = line.match(/if\s*\(url\.pathname\s*===\s*['"]([^'"]+)['"]\s*&&\s*req\.method\s*===\s*['"]([^'"]+)['"]/);
      if (match) {
  ...
  private categorizeEndpoint(path: string): EndpointInfo['category'] {
    ...
    if (path.includes('/withdrawal') || path.includes('/deposit') || path.includes('/queue/')) return 'financial';
...
// Role-based routes
${categories.filter(cat => !['auth', 'health'].includes(cat))
...
### By Category:
${[...new Set(this.endpoints.map(ep => ep.category))].map(cat => {
  const count = this.endpoints.filter(ep => ep.category === cat).length;
  return `- **${cat}**: ${count} endpoints`;
}).join('\n')}

### By Source:
${[...new Set(this.endpoints.map(ep => ep.sourceFile))].map(file => {
