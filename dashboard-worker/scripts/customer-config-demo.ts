...
class CustomerConfigDemo {
    ...
    for (const customer of this.testCustomers) {
      try {
        ...
        if (result.success) {
          ...
          console.log(`❌ Failed to create config for customer ${customer.customer_id}:`, result.error);
        }
      } catch (error) {
  ...
  async testCustomerConfigListing() {
    ...
    try {
      ...
      const agentResponse = await fetch(`${this.baseUrl}/api/customer-config/list?agentId=BLAKEPPH`);
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      const notFoundResponse = await fetch(`${this.baseUrl}/api/customer-config?customerId=NONEXISTENT`);
    ...
    try {
      ...
      if (result.success) {
        ...
        result.data.customerConfigs.forEach((config: CustomerConfig, index: number) => {
          ...
          console.log(`      Permissions: ${Object.values(config.permissions).filter(Boolean).length}/8 enabled`);
        });
      } else {
        console.log('❌ Failed to list customer configurations:', result.error);
      }
  ...
  async updateCustomerConfig() {
    ...
    const updates = {
      vip_status: {
        ...
        benefits: ['Priority Support', 'Special Rates', 'Exclusive Events', 'VIP Lounge Access', 'Personal Manager'],
        special_rates: 0.85,
        priority_support: true
      },
      betting_limits: {
