...
class WorkspaceCLI {
  ...
  private async splitWorkspaces(options: CLIOptions): Promise<void> {
    ...
    for (const [workspace, info] of result.workspaces) {
      const statusIcon = info.status === 'created' ? '✅' : 
                        info.status === 'updated' ? '🔄' : '❌';
  ...
  private async runBenchmarks(options: CLIOptions): Promise<void> {
    ...
    console.log(`🎯 Performance budgets: ${result.budgets.passed} passed, ${result.budgets.failed} failed`);
  ...
  private async runTests(options: CLIOptions): Promise<void> {
    ...
    const testOptions = {
      suites: options.suites?.filter(s => ['unit', 'integration', 'performance', 'e2e'].includes(s)) as any[],
  ...
  private async runEdgeCaseTests(options: CLIOptions): Promise<void> {
    ...
    console.log(`🧪 Total Tests: ${result.summary.totalPassed}/${result.summary.totalTests} passed`);
  ...
  private async showStatus(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      for (const [domain, packages] of status.workspaces) {
        ...
        if (options.verbose) {
          packages.packages.forEach(pkg => {
            const pkgIcon = pkg.buildStatus === 'success' ? '✅' : pkg.buildStatus === 'error' ? '❌' : '🔄';
      ...
      for (const [registry, regStatus] of status.publishing) {
        ...
        console.log(`   ${statusIcon} ${registry}: ${regStatus.published}/${regStatus.total} published`);
      ...
      if (status.benchmarks.lastRun) {
        const trendIcon = status.benchmarks.trend === 'up' ? '📈' : 
                         status.benchmarks.trend === 'down' ? '📉' : '➡️';
        ...
        console.log(`   Budgets: ${status.benchmarks.budgetsPassed}/${status.benchmarks.totalBudgets} passed`);
      } else {
      ...
      const { values, positionals } = parseArgs({
        ...
        options: {
          ...
          suites: { 
            type: 'string',
            multiple: true,
            description: 'Test/benchmark suites to run (unit,integration,performance,e2e,package,deployment)'
          },
  ...
  private async getGitStatus(workspaceDir: string): Promise<any> {
    try {
      const processManager = new (await import('./advanced-process-manager.ts')).AdvancedProcessManager();
