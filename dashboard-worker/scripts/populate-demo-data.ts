  ...
  try {
    ...
    for (const wager of demoWagers) {
      try {
        ...
        if (response.ok) {
          console.log(`   ✅ Created wager: ${wager.betId} ($${wager.stake} on ${wager.selection.substring(0, 30)}...)`);
        } else {
    ...
    console.log(`💰 Total Pending Amount: $${demoWagers.reduce((sum, w) => sum + w.stake, 0).toLocaleString()}`);
    console.log(`🎲 Total Potential Wins: $${demoWagers.reduce((sum, w) => sum + w.potentialWin, 0).toLocaleString()}`);
