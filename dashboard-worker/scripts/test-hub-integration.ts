...
class HubIntegrationTester {
  ...
  private async testHubConnection(): Promise<void> {
    await this.runTest('Hub Connection', async () => {
      ...
      return {
        ...
        connectedServices: connectionResult.connections.filter(c => c.status === 'connected')
          .length,
      };
    });
  }
  ...
  private async testDatabaseLinks(): Promise<void> {
    await this.runTest('Database Links Status', async () => {
      ...
      const connectedLinks = Object.values(testResults).filter(
  ...
  private printSummary(): void {
    ...
    this.results.forEach(result => {
      ...
      if (result.details && typeof result.details === 'object') {
        ...
        const preview = detailsStr.length > 200 ? detailsStr.substring(0, 200) + '...' : detailsStr;
