...
class DocumentationBuilder {
  ...
  async buildDocumentation(): Promise<DocumentationMetrics> {
    ...
    try {
      ...
      logger.success("DOC_BUILDER", "1.0.0", 
   ...
   */
  private async processDocumentationFile(filePath: string): Promise<{ processed: boolean; linked: boolean; size: number }> {
    try {
  ...
  private async generateBuildArtifacts(): Promise<void> {
    try {
      ...
      logger.error("DOC_BUILDER", "1.0.0", `Failed to generate build artifacts: ${error}`, "DOC003");
      throw error;
    }
  }
...
`;
  ...
  private async generateAPISummary(): Promise<any> {
    return {
      ...
      endpoints: {
        ...
        p2pQueue: [
          { path: '/api/p2p/queue/withdrawals', method: 'GET', description: 'Withdrawal queue data' },
          ...
          { path: '/api/p2p/queue/auto-match', method: 'POST', description: 'Auto-matching algorithm' },
          { path: '/api/p2p/queue/telegram/notify', method: 'POST', description: 'Telegram notifications' },
  ...
  private async updateIntegrationStatus(): Promise<void> {
    try {
      ...
      logger.warning("DOC_BUILDER", "1.0.0", 
        `Could not update hub integration status: ${error}`, "DOC004");
    }
  }
...
if (import.meta.main) {
  ...
  switch (command) {
    case 'build':
      ...
      builder.buildDocumentation().then(metrics => {
        console.log('\n📊 Build Results:');
