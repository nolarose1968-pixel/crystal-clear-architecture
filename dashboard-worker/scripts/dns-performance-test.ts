...
class DNSPerformanceTester {
  ...
  private async testConfiguration(ttl: number, verboseMode: string): Promise<DNSTestResult> {
    ...
    return {
      ...
      results: {
        ...
        cacheHitRate: finalStats.totalCount > 0 ? 
          (finalStats.cacheHitsCompleted / finalStats.totalCount) * 100 : 0,
  ...
  private printSummaryReport(results: DNSTestResult[]): void {
    ...
    // Group by TTL
    const ttlGroups = results.reduce((groups, result) => {
    ...
    for (const [ttl, ttlResults] of Object.entries(ttlGroups)) {
      ...
      ttlResults.forEach(result => {
        ...
        console.log(`  ${result.verboseMode.padEnd(5)}: ${avgTime}ms avg, ${hitRate}% cache hit, ${result.results.totalOperations} ops`);
      });
      ...
      console.log(`  🏆 Best: ${bestResult.verboseMode} mode (${bestResult.results.avgResponseTime.toFixed(0)}ms)`);
    }
    ...
    console.log(`  • Optimal configuration: TTL=${overallBest.ttl}s, Verbose=${overallBest.verboseMode}`);
    console.log(`  • Best performance: ${overallBest.results.avgResponseTime.toFixed(0)}ms average`);
    console.log(`  • Best cache hit rate: ${Math.max(...results.map(r => r.results.cacheHitRate)).toFixed(1)}%`);
  ...
  private async testRealWorldScenarios(): Promise<void> {
    ...
    console.log(`  Domains prefetched: Fire22=${coldStats.domains.length}, DB=${coldStats.databaseDomains.length}`);
