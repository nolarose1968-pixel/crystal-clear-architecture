...
class DatabaseConnector {
  ...
  async connect(): Promise<void> {
    return await runScript('database-connect', async () => {
      try {
        // Validate configuration
        const validation = validateConfig(this.config, dbConfigSchema);
        if (!validation.isValid) {
          throw createError('Invalid database configuration', {
        ...
        console.log(`✅ Connected to PostgreSQL database: ${this.config.database} on ${this.config.host}:${this.config.port}`);
        ...
        throw createError('Failed to connect to database', {
          scriptName: 'database-connector',
          operation: 'connect'
        }, {
          type: 'connection',
  ...
  async query<T = any>(sql: string, params: any[] = []): Promise<QueryResult<T>> {
    return await runScript('database-query', async () => {
      try {
        if (!this.isConnected) {
  ...
  async beginTransaction(): Promise<string> {
    return await runScript('database-transaction-begin', async () => {
      try {
        if (!this.isConnected) {
          throw new Error('Database not connected');
        }
        ...
        throw createError('Failed to begin transaction', {
          scriptName: 'database-connector',
  ...
  async executeInTransaction(transactionId: string, sql: string, params: any[] = []): Promise<QueryResult> {
    return await runScript('database-transaction-execute', async () => {
      try {
  ...
  async commitTransaction(transactionId: string): Promise<void> {
    return await runScript('database-transaction-commit', async () => {
      try {
        const transaction = this.transactions.get(transactionId);
        if (!transaction || transaction.status !== 'pending') {
          throw new Error(`Invalid transaction: ${transactionId}`);
        ...
        throw createError('Failed to commit transaction', {
          scriptName: 'database-connector',
  ...
  async rollbackTransaction(transactionId: string): Promise<void> {
    return await runScript('database-transaction-rollback', async () => {
      try {
        const transaction = this.transactions.get(transactionId);
        if (!transaction || transaction.status !== 'pending') {
          throw new Error(`Invalid transaction: ${transactionId}`);
        ...
        throw createError('Failed to rollback transaction', {
          scriptName: 'database-connector',
  ...
  async getDatabaseStats(): Promise<{ customers: number; transactions: number; bets: number }> {
    return await runScript('database-stats', async () => {
      try {
  ...
  async disconnect(): Promise<void> {
    return await runScript('database-disconnect', async () => {
      try {
        if (this.pool) {
          await this.pool.end();
        }
        ...
        throw createError('Failed to close database connections', {
          scriptName: 'database-connector',
