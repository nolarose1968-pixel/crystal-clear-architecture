...
async function showAllData() {
  return await ScriptRunner.run('data-viewer-main', async () => {
    ...
    if (metrics && metrics.size > 0) {
      for (const [scriptName, scriptMetrics] of metrics) {
        ...
        console.log(`   Success Rate: ${((scriptMetrics.successfulExecutions / scriptMetrics.totalExecutions) * 100).toFixed(1)}%`);
        console.log(`   Average Duration: ${scriptMetrics.averageDuration.toFixed(2)}ms`);
        console.log(`   Average Memory: ${(scriptMetrics.averageMemoryUsage / 1024 / 1024).toFixed(2)}MB`);
    ...
    if (history && history.length > 0) {
      ...
      recent.forEach((execution: any, index: number) => {
        ...
        console.log(`   ${index + 1}. ${status} ${execution.metadata.scriptName} (${duration}ms, ${memory}MB)`);
    ...
    return {
      ...
      totalExecutions: metrics ? Array.from(metrics.values()).reduce((sum: number, m: any) => sum + m.totalExecutions, 0) : 0
    };
  }, {
...
async function showSpecificScriptData(scriptName: string) {
  return await ScriptRunner.run('data-viewer-specific', async () => {
    const runner = ScriptRunner.getInstance();
    ...
    if (scriptMetrics) {
      ...
      const history = (runner as any).executionHistory;
      const scriptHistory = history.filter((execution: any) => execution.metadata.scriptName === scriptName);
...
async function exportDataToFile() {
  return await ScriptRunner.run('data-viewer-export', async () => {
    const runner = ScriptRunner.getInstance();
    ...
    const exportData = {
      ...
      summary: {
        ...
        totalExecutions: Array.from(metrics.values()).reduce((sum: number, m: any) => sum + m.totalExecutions, 0),
