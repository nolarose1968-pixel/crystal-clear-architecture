...
import { securityHealthCheck, getSecurityAuditLog } from '../src/api/middleware/enhanced-auth.middleware.js';
...
class SecurityHealthMonitor {
   ...
   */
  private determineOverallHealth(authHealth: any, analysis: any): 'healthy' | 'degraded' | 'unhealthy' {
    let score = 100;
  ...
  private displayReport(report: SecurityHealthReport): void {
    const statusColor = report.overall === 'healthy' ? '🟢' : 
