...
export const ERROR_INDEX = {
  ...
  "E2001": {
    ...
    severity: "CRITICAL",
    category: "DATABASE",
    description: "Database connection pool has reached maximum capacity and cannot serve new requests",
    solutions: [
  ...
  "E3002": {
    ...
    severity: "WARNING",
    category: "API",
    description: "Request rate exceeds configured API limits, check documentation for rate limit configuration",
    solutions: [
  ...
  "E6001": {
    ...
    severity: "CRITICAL",
    category: "SECURITY",
    description: "Multiple consecutive failed login attempts detected, potential brute force attack or unauthorized access attempt",
    solutions: [
  ...
  "E6002": {
    ...
    severity: "HIGH",
    category: "SECURITY", 
    description: "New connection detected from potentially suspicious IP address or geographic location",
    solutions: [
...
export function displayErrorIndex(): void {
  ...
  Object.entries(ERROR_CATEGORIES).forEach(([key, category]) => {
    console.log(`${category.color}${category.prefix}xxx${color("#ffffff", "css")} | ${category.name.padEnd(20)} | Range: ${category.range}`);
  ...
  Object.entries(ERROR_CATEGORIES).forEach(([categoryKey, category]) => {
    ...
    if (categoryErrors.length > 0) {
      ...
      categoryErrors.forEach(error => {
        ...
        console.log(`       Severity: ${color("#fbbf24", "css")}${error.severity}${color("#ffffff", "css")}`);
        ...
        if (error.occurrences > 0) {
          console.log(`       ${color("#ef4444", "css")}Occurrences: ${error.occurrences}${color("#ffffff", "css")}`);
        }
        console.log();
      });
    }
...
if (import.meta.main) {
  ...
  console.log("\n📊 **ERROR REPORT**");
  console.log("-".repeat(80));
  console.log(`Total Errors: ${color("#ef4444", "css")}${report.totalErrors}${color("#ffffff", "css")}`);
  
  console.log("\n🏷️ **Errors by Category:**");
  Object.entries(report.categoryCounts).forEach(([category, count]) => {
    console.log(`${category.padEnd(15)}: ${color("#ef4444", "css")}${count}${color("#ffffff", "css")}`);
  });
  
  console.log("\n⚠️ **Errors by Severity:**");
  Object.entries(report.severityCounts).forEach(([severity, count]) => {
    console.log(`${severity.padEnd(15)}: ${color("#f59e0b", "css")}${count}${color("#ffffff", "css")}`);
  });
  
  console.log("\n🔥 **Top 5 Errors:**");
  report.topErrors.forEach((error, index) => {
    if (error.occurrences > 0) {
      console.log(`${index + 1}. ${color("#ef4444", "css")}${error.code}${color("#ffffff", "css")} - ${error.message} (${color("#fbbf24", "css")}${error.occurrences}${color("#ffffff", "css")} times)`);
    }
  });
