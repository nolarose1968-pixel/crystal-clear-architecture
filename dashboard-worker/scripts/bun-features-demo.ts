...
class BunFeaturesDemo {
  ...
  async demoCustomUserAgent(): Promise<void> {
    ...
    try {
      ...
      if (customData['user-agent'].includes('Fire22')) {
        ...
        console.log('   This means you ran: bun --user-agent "Fire22-*" run scripts/bun-features-demo.ts');
      } else {
  ...
  async runAllDemos(): Promise<void> {
    ...
    try {
      ...
      console.log('\n💡 Next Steps:');
      console.log('   1. Test with custom User-Agent: bun --user-agent "Fire22-Demo/3.0.8" run scripts/bun-features-demo.ts');
