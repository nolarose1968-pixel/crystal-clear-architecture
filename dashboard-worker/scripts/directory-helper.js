...
class DirectoryHelper {
  ...
  async generateDiagnosticReport(directoryPath, results) {
    ...
    console.log(
      `   💾 Compression Ratio: ${(((buffer.length - compressed.length) / buffer.length) * 100).toFixed(1)}%`
    );
      ...
      for (const item of contents) {
        ...
        if (!stats.isDirectory()) {
          try {
            ...
            if (itemData.category === 'text' && stats.size < 1024 * 1024) {
              // Only for files < 1MB
              try {
      ...
      try {
        ...
        if (analysis) {
          ...
          const summaryTable = [
            {
              Type: 'Files',
              Count: analysis.summary.files,
              Size: `${analysis.summary.size} bytes`,
            },
