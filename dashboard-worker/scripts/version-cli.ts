...
class VersionCLI {
  ...
  private async listPackages(options: CLIOptions): Promise<void> {
    ...
    const packages = Array.from(workspace.packages.entries())
  ...
  private async showBuildConstants(options: CLIOptions): Promise<void> {
    ...
    if (workspace.buildConstants.WORKSPACE_COMPONENTS) {
      ...
      for (const component of workspace.buildConstants.WORKSPACE_COMPONENTS as any[]) {
        ...
        console.log(`     Responsibilities: ${component.responsibilities.slice(0, 2).join(', ')}${component.responsibilities.length > 2 ? '...' : ''}`);
      }
      console.log('');
    }
    ...
    if (options.output) {
      await Bun.write(options.output, JSON.stringify({
