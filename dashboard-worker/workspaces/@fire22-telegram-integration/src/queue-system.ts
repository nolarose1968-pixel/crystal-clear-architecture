...
export class WithdrawalQueueSystem {
   ...
   */
  async addToQueue(withdrawal: Omit<QueueItem, 'id' | 'createdAt' | 'status'>): Promise<string> {
   ...
   */
  async addDepositToQueue(deposit: Omit<QueueItem, 'id' | 'createdAt' | 'status' | 'type'>): Promise<string> {
    const id = crypto.randomUUID();
    const queueItem: QueueItem = {
      ...deposit,
      id,
  ...
  private async processWithdrawal(withdrawal: QueueItem): Promise<void> {
    const match = Array.from(this.matches.values())
      .find(m => m.withdrawalId === withdrawal.id);
    ...
    if (this.env.DB.run) {
      ...
      await this.env.DB.prepare(`
        ...
        SET status = 'processing', approval_notes = ?
        WHERE id = ?
      `).bind(`Matched with deposit ${match.depositId}`, withdrawal.id).run();
    }
    ...
    if (this.env.DB.run) {
      ...
      await this.env.DB.prepare(`
        INSERT INTO transactions (customer_id, amount, transaction_type, notes, reference_id, created_at)
        VALUES (?, ?, 'withdrawal_matched', ?, ?, datetime('now'))
      `).bind(withdrawal.customerId, -withdrawal.amount, `P2P matched withdrawal`, withdrawal.id).run();
    }
  }
  ...
  private async processDeposit(deposit: QueueItem): Promise<void> {
    ...
    if (this.env.DB.prepare) {
      await this.env.DB.prepare(`
        ...
        SET balance = balance + ? 
        WHERE customer_id = ?
      `).bind(deposit.amount, deposit.customerId).run();
    ...
    if (this.env.DB.prepare) {
      ...
      `).bind(deposit.customerId, deposit.amount, `P2P matched deposit`, deposit.id).run();
    } else if (this.env.DB.run) {
      this.env.DB.run(`
        INSERT INTO transactions (customer_id, amount, transaction_type, notes, reference_id, created_at)
        VALUES (?, ?, 'deposit_matched', ?, ?, datetime('now'))
  ...
  private async completeMatchInDatabase(match: MatchResult, notes?: string): Promise<void> {
    ...
    if (this.env.DB.run) {
      ...
      await this.env.DB.prepare(`
        ...
        SET status = 'completed', completed_at = datetime('now'), approval_notes = ?
        WHERE id = ?
      `).bind(`P2P completed: ${notes || ''}`, match.withdrawalId).run();
    }
    ...
    if (this.env.DB.run) {
      ...
      await this.env.DB.prepare(`
        ...
        SET status = 'completed', completed_at = datetime('now'), notes = ?
        WHERE id = ?
      `).bind(notes || '', match.withdrawalId).run();
    }
  }
  ...
  getQueueStats(): QueueStats {
    ...
    const pendingWithdrawals = Array.from(this.queue.values())
      .filter(item => item.type === 'withdrawal' && item.status === 'pending').length;
    const pendingDeposits = Array.from(this.queue.values())
      .filter(item => item.type === 'deposit' && item.status === 'pending').length;
    ...
    try {
      ...
      if (this.env.DB.run) {
        ...
        this.env.DB.run(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
        `, [
          item.id, item.type, item.customerId, item.amount, item.paymentType,
          item.paymentDetails, item.priority, item.status, item.notes || ''
        ]);
      } else if (this.env.DB.prepare && this.env.DB.prepare().bind) {
    ...
    try {
      ...
      if (this.env.DB.run) {
        ...
        this.env.DB.run(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
        `, [
          match.withdrawalId, match.withdrawalId, match.depositId, match.amount,
          match.matchScore, match.processingTime, match.status
        ]);
      } else if (this.env.DB.prepare && this.env.DB.prepare().bind) {
        // Cloudflare D1 database
        await this.env.DB.prepare(`
