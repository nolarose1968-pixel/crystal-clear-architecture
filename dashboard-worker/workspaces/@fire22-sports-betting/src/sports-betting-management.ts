...
export interface SportsEvent {
  ...
  odds: SportsOdds;
  riskLevel: RiskLevel;
  customerTierAccess: CustomerTier[];
  isActive: boolean;
  lastUpdated: Date;
}
...
export interface CustomerTierProfile {
  id: string;
  playerId: string;
  currentTier: CustomerTier;
  points: number;
  requirements: CustomerTierRequirements;
  benefits: CustomerTierBenefits;
  status: 'active' | 'suspended' | 'upgrading' | 'downgrading';
  joinedAt: Date;
  lastUpdated: Date;
}

export interface CustomerTierRequirements {
  minBalance: number;
  minVolume: number;
  minBets: number;
  minWinRate: number;
...
export interface CustomerTierBenefits {
...
export class SportsBettingManagementSystem {
  ...
  private customerTierProfiles: Map<string, CustomerTierProfile> = new Map();
