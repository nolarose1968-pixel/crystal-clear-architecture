 ...
 */
function applyScopeRestriction(scope, requestParams) {
    if (!scope)
 ...
 */
export async function canUserAccess(user, resource, action) {
    if (!user)
        return false;
 ...
 */
export async function logAuthorizationEvent(user, action, resource, allowed, reason) {
    const event = {
        timestamp: new Date(),
        userId: user?.id,
        userRole: user?.role,
