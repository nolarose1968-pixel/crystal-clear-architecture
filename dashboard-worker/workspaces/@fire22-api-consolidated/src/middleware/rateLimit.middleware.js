 ...
 */
function getRoleLimits(role, roleBasedLimits) {
    if (!roleBasedLimits) {
 ...
 */
function defaultLimitResponse(request, retryAfter) {
 ...
 */
export function rateLimiter(options = { max: 100, windowMs: 60000 }) {
    const { max: defaultMax, windowMs: defaultWindowMs, keyGenerator = defaultKeyGenerator, roleBasedLimits, skip, onLimitReached = defaultLimitResponse, standardHeaders = true, store = memoryStore } = options;
 ...
 */
export function createCloudflareKVStore(kv) {
    return new CloudflareKVStore(kv);
}
/**
 * Get rate limit status for a key
 */
export async function getRateLimitStatus(key, store = memoryStore) {
    return await store.get(key);
}
/**
 * Reset rate limit for a key
 */
export async function resetRateLimit(key, store = memoryStore) {
