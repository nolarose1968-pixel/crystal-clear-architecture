...
${colors.cyan}╰${'─'.repeat(width - 2)}╯${colors.reset}`;
  ...
  static drawSection(title: string, content: string[], width: number = 70): string {
    ...
    lines.push(`${colors.blue}╭${'─'.repeat(titleLeft)}${colors.reset} ${colors.bright}${title}${colors.reset} ${colors.blue}${'─'.repeat(titleRight)}╮${colors.reset}`);
    
    // Content lines
    content.forEach(line => {
      const contentWidth = this.stripAnsi(line).length;
      const padding = width - contentWidth - 2;
      lines.push(`${colors.blue}│${colors.reset} ${line}${' '.repeat(Math.max(0, padding))} ${colors.blue}│${colors.reset}`);
    });
...
class CustomerAPITester {
  ...
  async testGetHierarchy(customerId: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
  ...
  async testGetBettingHistory(page: number = 1, limit: number = 10): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const response = await fetch(`${this.baseURL}/api/customer/betting-history?page=${page}&limit=${limit}`, {
        method: 'GET',
        headers: this.getHeaders()
      });
  ...
  async testUpdateProfile(profileData: any): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const response = await fetch(`${this.baseURL}/api/customer/profile`, {
        method: 'PUT',
        headers: this.getHeaders(),
...
async function runInteractiveCLI() {
  ...
  console.log(AestheticBox.drawSection('Configuration', [
  ...
  console.log(AestheticBox.drawSection('Test 1: Get Profile', [
  ...
  if (config.verbose && profileResult.success) {
    console.log(`${colors.dim}Response:${colors.reset}`, JSON.stringify(profileResult.data, null, 2));
  }
  
  // ╭────────────── Test 2: Get Hierarchy ──────────────╮
  console.log(AestheticBox.drawDivider());
  console.log(AestheticBox.drawSection('Test 2: Get Hierarchy', [
  ...
  // ╭────────────── Test 3: Get Balance ──────────────╮
  console.log(AestheticBox.drawDivider());
  console.log(AestheticBox.drawSection('Test 3: Get Balance', [
  ...
  console.log(AestheticBox.drawDivider());
  console.log(AestheticBox.drawSection('Test 4: Betting History', [
  ...
  console.log(AestheticBox.drawDivider());
  console.log(AestheticBox.drawSection('Test 5: Update Profile', [
  ...
  results.forEach(result => {
    ...
    summaryContent.push(`${statusIcon} ${colors.cyan}${result.endpoint}${colors.reset}: ${statusColor}${result.message}${colors.reset}`);
  ...
  // Final statistics
  console.log();
  console.log(AestheticBox.drawSection('Statistics', [
