...
class RetryManager {
  private static async retry<T>(
    ...
    throw new RetryExhaustedError(`${operationName} failed after ${maxAttempts} attempts: ${lastError!.message}`);
  }
  ...
  static async withTransaction<T>(
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (error instanceof Error && error.message.includes('SQLITE_BUSY') && attempt < maxRetries) {
          const delay = 100 * Math.pow(2, attempt);
          console.warn(`Transaction ${operationName} retry ${attempt} after ${delay}ms due to SQLITE_BUSY`);
          await new Promise(resolve => setTimeout(resolve, delay));
          continue;
        }
        
        throw new DatabaseError(`Transaction ${operationName} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
...
class CircuitBreakerOpenError extends RegistryError {
  constructor(serviceName: string, details?: Record<string, any>) {
    const code = serviceName.includes('database') ? 'CB6001' : 
                 serviceName.includes('storage') ? 'CB6002' : 'CB6001';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      metrics.timing('request_duration', duration, { method: request.method, path, status: String(response.status) });
...
async function handleRequest(request: Request, env: Env, path: string): Promise<Response> {
  ...
  if (path === '/-/stats' && method === 'GET') {
    return await RetryManager.withRetry(
      ...
      maxRetries,
      'get_registry_stats'
    ).then(stats => new Response(JSON.stringify(stats), {
...
async function handlePackageSearch(request: Request, env: Env, maxRetries: number): Promise<Response> {
  const url = new URL(request.url);
...
async function handlePackagePublish(request: Request, env: Env, path: string, maxRetries: number): Promise<Response> {
  ...
  if (!isAllowedScope) {
    throw new ValidationError(`Package ${packageName} not in allowed scopes: ${allowedScopes.join(', ')}`);
  }
...
async function handlePackageGet(request: Request, env: Env, path: string, maxRetries: number): Promise<Response> {
  const packageName = path.slice(1);
...
async function handlePackageDownload(request: Request, env: Env, path: string, maxRetries: number): Promise<Response> {
  ...
  try {
    ...
    await dbCircuitBreaker.execute(async () => {
      return await db
        .prepare('INSERT OR IGNORE INTO downloads (package, version, downloaded_at) VALUES (?, ?, ?)')
