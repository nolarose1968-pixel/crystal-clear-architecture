...
async function handleRequest(request: Request, registry: CloudflareRegistry, path: string): Promise<Response> {
...
async function handlePackagePublish(request: Request, registry: CloudflareRegistry, path: string): Promise<Response> {
  try {
...
async function handlePackageGet(request: Request, registry: CloudflareRegistry, path: string): Promise<Response> {
...
async function handlePackageDownload(request: Request, registry: CloudflareRegistry, path: string): Promise<Response> {
  try {
...
class CloudflareRegistry extends Fire22RegistryServer {
  ...
  protected async loadPackages(): Promise<void> {
    try {
      const { results } = await this.env.REGISTRY_DB
