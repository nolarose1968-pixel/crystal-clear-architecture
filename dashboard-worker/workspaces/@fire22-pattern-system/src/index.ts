        ...
        overallStats: {
          ...
          overallCompressionRatio: compressionData.length > 0 ? 
            ((compressionData.reduce((sum, c) => sum + c.originalSize, 0) - 
             ...
             compressionData.reduce((sum, c) => sum + c.originalSize, 0) * 100).toFixed(2) + '%' : '0%'
        }
      };
    }
  ...
  async fileStreamingWorkflow() {
    ...
    if (streamResult.success && streamResult.analysis) {
      ...
      if (streamResult.analysis.dataTypes.length > 0) {
        ...
        const dataTable = patternWeaver.applyPattern('TABULAR', 
          streamResult.analysis.dataTypes.map(type => ({ 'Data Type': type, Status: '✅ Detected' }))
        );
