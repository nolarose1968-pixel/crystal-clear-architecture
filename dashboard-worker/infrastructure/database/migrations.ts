...
export class MigrationRunner {
  ...
  async getAppliedMigrations(): Promise<number[]> {
    ...
    if (this.db instanceof Database) {
      const result = this.db.query("SELECT version FROM schema_migrations ORDER BY version").all() as any[];
    ...
    return [
      {
        ...
        up: async (db) => {
          ...
          const createTasksQuery = this.dbManager.getConfig().adapter === 'sqlite' 
            ? `
      ...
      },
      {
        ...
        up: async (db) => {
          if (db instanceof Database) {
            ...
            const createActivitiesQuery = this.dbManager.getConfig().adapter === 'sqlite'
            ? `
            ...
            db.exec("CREATE INDEX IF NOT EXISTS idx_task_activities_task ON task_activities(task_uuid)");
          } else {
            throw new Error('Non-SQLite databases not yet supported');
          }
        },
