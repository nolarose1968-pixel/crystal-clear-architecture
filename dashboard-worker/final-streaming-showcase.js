...
`;
  ...
  const comprehensiveResult = await patternWeaver.applyPattern('STREAM', {
    ...
    processor: async (text, chunk, index) => {
      ...
      const codeMetrics = {
        ...
        functions: (
          text.match(/(?:function\s+\w+|async\s+function\s+\w+|\w+\s*\([^)]*\)\s*{)/g) || []
        ).length,
      ...
      const complexity =
        codeMetrics.conditions + codeMetrics.loops + codeMetrics.switches + codeMetrics.functions;
  ...
  if (comprehensiveResult.success) {
    ...
    if (binaryResults.length > 0) {
      const avgEntropy =
        binaryResults.reduce((sum, r) => sum + parseFloat(r.entropy), 0) / binaryResults.length;
      const avgPrintable =
        binaryResults.reduce((sum, r) => sum + parseFloat(r.printableRatio), 0) /
  ...
  const benchmarkConfigs = [
    ...
    {
      name: 'Analysis Only',
      config: { source: 'file', filePath: './complex-test-file.ts', analysis: true },
    },
    {
  ...
  console.log(
    '│ Configuration   │ Total Time   │ Stream Time  │ Overhead     │ Throughput     │ Success │'
  );
  console.log(
    '├─────────────────┼──────────────┼──────────────┼──────────────┼────────────────┼─────────┤'
  );
