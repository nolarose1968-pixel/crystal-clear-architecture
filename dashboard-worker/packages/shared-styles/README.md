...
**Enterprise-grade CSS highlighting with OKLCH colors, Bun bundling, and comprehensive theming**
...
--fire22-status-warning: oklch(75% 0.20 85);    /* Warning yellow */
...
@media (prefers-color-scheme: dark) {
  ...
  --fire22-status-success: oklch(70% 0.20 145);
...
```css
:root {
  --fire22-status-custom: oklch(65% 0.20 180); /* Custom cyan */
}
```
...
expect(getComputedStyle(packageCard).boxShadow).toContain('oklch(70% 0.15 240)');
```
...
*Part of the Fire22 Dashboard ecosystem - built with Bun, designed for scale*