...
export const FIRE22_STATUS_COLORS: Record<string, Fire22StatusColor> = {
  success: {
    name: 'success',
    oklch: { lightness: 0.60, chroma: 0.1, hue: 120 },
  ...
  warning: {
    name: 'warning',
    oklch: { lightness: 0.80, chroma: 0.15, hue: 80 },
  ...
  error: {
    name: 'error',
    oklch: { lightness: 0.60, chroma: 0.22, hue: 25 },
  ...
  info: {
    name: 'info',
    oklch: { lightness: 0.70, chroma: 0.15, hue: 260 },
  ...
  default: {
    name: 'default',
    oklch: { lightness: 0.70, chroma: 0.15, hue: 240 },
  ...
  draft: {
    name: 'draft',
    oklch: { lightness: 0.60, chroma: 0.08, hue: 280 },
  ...
  archived: {
    name: 'archived',
    oklch: { lightness: 0.50, chroma: 0.05, hue: 200 },
...
export const FIRE22_BRAND_COLORS = {
  primary: { lightness: 0.70, chroma: 0.25, hue: 30 },
...
export function getHighlightColor(status: keyof typeof FIRE22_STATUS_COLORS): string {
  ...
  if (!statusColor) {
    throw new Error(`Unknown Fire22 status: ${status}. Available: ${Object.keys(FIRE22_STATUS_COLORS).join(', ')}`);
  }
...
export function generateHighContrastVariant(color: OKLCHColor): OKLCHColor {
  return {
    lightness: color.lightness > 0.5 ? Math.min(color.lightness + 0.1, 0.95) : Math.max(color.lightness - 0.1, 0.05),
 ...
 */
export function validateWCAGCompliance(color: OKLCHColor, background: 'light' | 'dark' = 'light'): boolean {
 ...
 */
export function generateColorPalette(): Array<{name: string; css: string; hex: string; wcag: boolean}> {
  ...
  switch (command) {
    ...
    case 'validate':
      Object.entries(FIRE22_STATUS_COLORS).forEach(([name, color]) => {
        ...
        console.log(`${name}: Light BG: ${lightValid ? '✅' : '❌'}, Dark BG: ${darkValid ? '✅' : '❌'}`);
      });
      break;
    ...
    case 'create':
      ...
      if (!name || !l || !c || !h) {
        console.error('Usage: bun color-utils.ts create <name> <lightness> <chroma> <hue> <description>');
        process.exit(1);
      }
