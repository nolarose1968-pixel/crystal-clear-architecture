...
-----END RSA PRIVATE KEY-----`;
  ...
  describe("Recommendations Generation", () => {
    it("should generate appropriate recommendations", async () => {
      ...
      expect(result.recommendations).toContain("🔐 Use Bun.secrets for credential storage instead of environment variables");
      expect(result.recommendations).toContain("🛡️ Enable Bun's built-in security scanner in bunfig.toml");
      expect(result.recommendations).toContain("📦 Regularly update dependencies to latest versions");
    });
