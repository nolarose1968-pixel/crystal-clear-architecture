...
class SecurityScannerBenchmark {
  ...
  async benchmarkSecretScanning(): Promise<BenchmarkResult> {
    ...
    const testFiles = [
      ...
      { name: "utils.ts", content: "export function formatDate(date: Date) { return date.toISOString(); }" }
    ];
    ...
    return this.runBenchmark(
      "Secret Scanning",
      () => this.scanner.scanSecrets(tempDir),
      300
    );
  }
  ...
  generateReport(): string {
    ...
    report += "\n🏅 Performance Grades\n";
    report += "-" .repeat(40) + "\n";
    report += avgScanTime < 5 ? "✅ Scan Speed: A+ (Sub-5ms)\n" : avgScanTime < 10 ? "✅ Scan Speed: A (Sub-10ms)\n" : "⚠️ Scan Speed: B\n";
