  ...
  return (
    <article className={`fire22-highlight fire22-package-card ${statusClass}`}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <p style={{ fontSize: "12px", color: "#e0e6ed", margin: "8px 0" }}>
          {description}
        </p>
        ...
        <div style={{ 
          ...
          <span style={{
            ...
            borderRadius: "4px",
            fontWeight: "bold",
            background: statusClass === 'success' ? "rgba(74, 222, 128, 0.2)" :
                       statusClass === 'error' ? "rgba(239, 68, 68, 0.2)" :
                       "rgba(251, 191, 36, 0.2)",
            color: statusClass === 'success' ? "#4ade80" :
                   statusClass === 'error' ? "#ef4444" :
                   "#fbbf24",
            border: `1px solid ${
              statusClass === 'success' ? "rgba(74, 222, 128, 0.3)" :
              statusClass === 'error' ? "rgba(239, 68, 68, 0.3)" :
              "rgba(251, 191, 36, 0.3)"
            }`
          }}>
            {status.toUpperCase()}
...
export default function PackageReviewGrid() {
  const telegramPackages = packages.filter(p => 
    p.name.includes('telegram') || p.name.includes('queue') || p.name.includes('multilingual')
  );
  
  const corePackages = packages.filter(p => 
    p.name.includes('core') || p.name.includes('api') || p.name.includes('pattern')
  );
  
  const specializedPackages = packages.filter(p => 
    p.name.includes('sports') || p.name.includes('security') || p.name.includes('benchmarks')
  );
