...
describe('Fire22SecurityScanner', () => {
  ...
  describe('Fatal Level Advisories', () => {
    ...
    test('should detect typosquatting attempts', async () => {
      ...
      const typosquatAdvisories = result.advisories.filter(
        advisory => advisory.title.includes('typosquatting')
      );
    ...
    test('should detect known CVE vulnerabilities in fatal range', async () => {
      ...
      const cveAdvisories = result.advisories.filter(
        advisory => advisory.cve === 'CVE-2021-23337'
      );
  ...
  describe('Warning Level Advisories', () => {
    ...
    test('should warn about pre-release versions', async () => {
      ...
      const prereleaseAdvisories = result.advisories.filter(
        advisory => advisory.title.includes('Pre-release')
      );
  ...
  describe('Context-Aware Scanning', () => {
    test('should consider production vs development context', async () => {
      const request: ScanRequest = {
        packages: [
          { name: 'debug-utils', version: '1.0.0-alpha.1' }
        ],
        context: {
          production: true,
      ...
      const prereleaseWarnings = result.advisories.filter(
        advisory => advisory.title.includes('Pre-release')
      );
