...
class ScopedPackageSecurityDemo {
  ...
  async demoScopedPackages() {
    ...
    const otherScopedPackages = scopedRequest.packages.filter(p => p.name.startsWith('@') && !p.name.startsWith('@fire22/'));
    ...
    if (scopedAdvisories.length > 0) {
      ...
      scopedAdvisories.forEach(advisory => {
        console.log(`   ${advisory.level === 'fatal' ? '🚨' : '⚠️'} ${advisory.package}: ${advisory.title}`);
      });
      console.log('');
    }
  ...
  async demoRegistryConfiguration() {
    ...
    const registryRequest: ScanRequest = {
      packages: [
        ...
        { name: '@fire22/security-core', version: '1.0.0', registry: 'https://fire22.workers.dev/registry' },
    ...
    console.log(`   Private registry packages: ${registryRequest.packages.filter(p => p.registry?.includes('fire22')).length}`);
    console.log(`   Public registry packages: ${registryRequest.packages.filter(p => !p.registry?.includes('fire22')).length}`);
  ...
  async demoScopedPolicies() {
    ...
    customScopeRequest.packages.forEach(pkg => {
      ...
      console.log(`   ${icon} ${pkg.name}: ${
        pkg.name.startsWith('@fire22/') ? 'Auto-trusted (Fire22 scope)' :
        pkg.name.startsWith('@partner/') ? 'Partner scope - enhanced scanning' :
        hasAdvisory ? 'Security issues detected' :
  ...
  private isScopeSquat(suspectScope: string, legitimateScope: string): boolean {
    ...
    return patterns.some(pattern => suspect === pattern) ||
