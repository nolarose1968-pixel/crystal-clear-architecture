...
class SecurityScannerDemo {
  ...
  async demoFatalThreats() {
    ...
    console.log('📦 Scanning packages:', fatalRequest.packages.map(p => `${p.name}@${p.version}`).join(', '));
  ...
  async demoWarningScenarios() {
    ...
    console.log('📦 Scanning packages:', warningRequest.packages.map(p => `${p.name}@${p.version}`).join(', '));
  ...
  async demoFire22Integration() {
    ...
    console.log(`   @fire22/* Packages: ${fire22Request.packages.filter(p => p.name.startsWith('@fire22/')).length} (auto-trusted)`);
    console.log(`   External Packages: ${fire22Request.packages.filter(p => !p.name.startsWith('@fire22/')).length}`);
