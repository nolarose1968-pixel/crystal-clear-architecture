...
export class Fire22SecurityScanner {
  ...
  async scan(request: ScanRequest): Promise<ScanResult> {
    try {
      ...
      throw new Error(`Fire22 Security Scanner failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
    ...
    for (const threat of this.threatFeed) {
      ...
      if (threat.package === packageName && this.isVersionVulnerable(version, threat.version)) {
        advisories.push({
          ...
          description: threat.description || `Package ${packageName} has been identified as containing ${threat.categories.join(', ')}`,
    ...
    for (const policy of this.fire22Policies) {
      if (policy.pattern.test(packageName)) {
        ...
        advisories.push({
          ...
          recommendation: policy.level === 'fatal' 
  ...
  private async updateThreatFeed(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Threat feed validation failed: ${error instanceof Error ? error.message : 'Invalid data'}`);
    }
  }
   ...
   */
  async checkPackageIntegrity(packageName: string, version: string, expectedHash?: string): Promise<boolean> {
