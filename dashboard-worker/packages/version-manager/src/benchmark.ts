...
async function main() {
  ...
  const testVersions = [
    '1.0.0', '2.0.0-alpha.1', '1.2.0', '2.0.0', '1.10.0',
    '2.0.0-beta.1', '1.1.0', '2.0.0-rc.1', '3.0.0', '1.0.0-alpha.1'
  ];
  ...
  console.log(`Memory for 10,000 version objects: ${colors.cyan(formatNumber(Math.round(memoryDelta / 1024)))}KB`);
  ...
  const fastestParsing = results.filter(r => r.operation.includes('Parse')).reduce((min, curr) => 
    curr.avgTime < min.avgTime ? curr : min
  );
  
  const fastestComparison = results.filter(r => r.operation.includes('Compare')).reduce((min, curr) => 
    curr.avgTime < min.avgTime ? curr : min
  );
  
  const fastestSatisfies = results.filter(r => r.operation.includes('satisfies')).reduce((min, curr) => 
    curr.avgTime < min.avgTime ? curr : min
  );
  ...
  console.log(`${colors.green('🏆')} Fastest Comparison: ${colors.bold(fastestComparison.operation)}`);
  console.log(`    ${formatTime(fastestComparison.avgTime)} per operation`);
  
  console.log(`${colors.green('🏆')} Fastest Satisfaction: ${colors.bold(fastestSatisfies.operation)}`);
  ...
  if (comparisonResult) checkTarget(comparisonResult.avgTime, targets.comparison, 'Version Comparison');
  if (satisfiesResult) checkTarget(satisfiesResult.avgTime, targets.satisfaction, 'Range Satisfaction');
  ...
  console.log(`Total benchmark time: ${colors.cyan((results.reduce((sum, r) => sum + r.totalTime, 0) / 1000).toFixed(2))}s`);
}
