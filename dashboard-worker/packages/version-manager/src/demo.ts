...
async function main() {
  ...
  if (consistency.consistent) {
    ...
    for (const issue of consistency.inconsistencies) {
      console.log(`  ${issue.package}: ${colors.red(issue.version)} (expected: ${colors.green(issue.expected)})`);
    }
  }
  ...
  console.log(`  Parsing:     ${colors.green(parseTime.toFixed(2))}ms total, ${colors.cyan((parseTime/iterations*1000).toFixed(3))}μs per op`);
  console.log(`  Comparison:  ${colors.green(compareTime.toFixed(2))}ms total, ${colors.cyan((compareTime/iterations*1000).toFixed(3))}μs per op`);
  console.log(`  Satisfaction: ${colors.green(satisfiesTime.toFixed(2))}ms total, ${colors.cyan((satisfiesTime/iterations*1000).toFixed(3))}μs per op`);
