...
## Overview
Professional sports trading platform with advanced Closing Line Value (CLV) analysis, sharp detection, and automated trading algorithms.
...
export class SportsLinesManager {
  ...
  private detectSteamMove(eventId: string, update: LineUpdate): boolean {
    ...
    if (recentMoves.length >= 3) {
      ...
      const consistentMoves = recentMoves.filter(m => 
        Math.sign(m.movement) === direction
      ).length;
  ...
  private calculateConsensus(movements: LineMovement[]): ConsensusLine {
    ...
    const currentLines = books.map(book => 
...
export class MarketMakingEngine {
  ...
  async generateMarket(event: SportingEvent): Promise<MarketLines> {
    ...
    return {
      moneyline: {
        home: this.applyMargin(fairLines.moneyline.home, margins.moneyline, riskAdjustment.home),
        away: this.applyMargin(fairLines.moneyline.away, margins.moneyline, riskAdjustment.away)
      },
      spread: {
   ...
   */
  async adjustForAction(event: string, action: BettingAction): Promise<LineAdjustment> {
...
export class CLVAnalysisEngine {
  ...
  async generateCLVReport(
    ...
    return {
      ...
      positiveCLVRate: validCLVs.filter(r => r.clv > 0).length / validCLVs.length,
  ...
  async detectSharpPatterns(customer: Customer): Promise<SharpAnalysis> {
    const recentBets = await this.betHistory.getBetsByCustomer(
      customer.id, 
      { days: 30 }
    );
