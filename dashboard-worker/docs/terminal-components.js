...
class TerminalComponents {
    ...
    updateMetric(id, value, trend = null, status = null) {
        const metricElement = document.getElementById(id) || document.querySelector(`[data-metric="${id}"]`);
        ...
        if (trend) {
            ...
            if (trendElement) {
                ...
                trendElement.textContent = this.getTrendIcon(trend) + ' ' + trendElement.textContent.split(' ').slice(1).join(' ');
            }
        }
    ...
    setProgress(id, value, animated = true) {
        ...
        const progressElement = document.getElementById(id) || document.querySelector(`[data-progress="${id}"]`);
    ...
    toggleState(id, state) {
        const element = document.getElementById(id) || document.querySelector(`[data-component="${id}"]`);
        if (!element) return;
    ...
    updateLiveStatus(statusElement) {
        ...
        const currentState = states.find(state => statusElement.classList.contains(`terminal-status-dot--${state}`));
...
class TerminalForm {
    ...
    setupValidation() {
        const inputs = this.element.querySelectorAll('.terminal-form__input[required], .terminal-form__textarea[required]');
...
class TerminalTable {
    ...
    setupSorting() {
        const headers = this.element.querySelectorAll('.terminal-table__row--header .terminal-table__cell');
