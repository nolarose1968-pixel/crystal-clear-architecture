...
class AssetOptimizer {
  ...
  async optimizeAsset(assetPath, buildDir) {
    ...
    try {
      ...
      console.log(
        `   ${this.formatBytes(originalSize)} → ${this.formatBytes(optimizedSize)} (${savingsPercent}% saved)\n`,
      );
    } catch (error) {
  ...
  generateReport() {
    ...
    console.log(
      "📊 Optimization report saved to build/optimization-report.json",
    );
  }
  ...
  getRecommendations() {
    ...
    if (this.stats.savings < 10000) {
      recommendations.push(
        "Consider implementing more aggressive minification",
      );
    }
  ...
  showSummary() {
    ...
    const compressionRatio =
      this.stats.original > 0
        ? (
            ((this.stats.original - this.stats.optimized) /
              this.stats.original) *
            100
          ).toFixed(1)
        : "0.0";
    ...
    console.log(
      '💡 Run: git add build/ && git commit -m "Optimize assets" && git push',
    );
  }
